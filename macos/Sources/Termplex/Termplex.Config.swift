...
extension Termplex {
    /// Maps to a `termplex_config_t` and the various operations on that.
    class Config: ObservableObject {
        // The underlying C pointer to the Termplex config structure. This
        // should never be accessed directly. Any operations on this should
        // be called from the functions on this or another class.
        private(set) var config: termplex_config_t? {
            didSet {
                // Free the old value whenever we change
                guard let old = oldValue else { return }
                termplex_config_free(old)
            }
        }
        ...
        var errors: [String] {
            ...
            var diags: [String] = []
            let diagsCount = termplex_config_diagnostics_count(cfg)
            for i in 0..<diagsCount {
                let diag = termplex_config_get_diagnostic(cfg, UInt32(i))
        ...
        convenience init(clone config: termplex_config_t) {
            self.init(config: termplex_config_clone(config))
        }
        ...
        static func loadConfig(at path: String?, finalize: Bool) -> termplex_config_t? {
            // Initialize the global configuration.
            guard let cfg = termplex_config_new() else {
                logger.critical("termplex_config_new failed")
                return nil
            }
...
#if os(macOS)
            if let path {
                termplex_config_load_file(cfg, path)
            } else {
                termplex_config_load_default_files(cfg)
            }

            // We only load CLI args when not running in Xcode because in Xcode we
            // pass some special parameters to control the debugger.
            if !isRunningInXcode() {
                termplex_config_load_cli_args(cfg)
            }

            termplex_config_load_recursive_files(cfg)
#endif
        ...
        /// in the Termplex configuration. For example `keybind = cmd+q=quit` in Termplex
        ...
        func keyboardShortcut(for action: String) -> KeyboardShortcut? {
            ...
            let trigger = termplex_config_trigger(cfg, action, UInt(action.lengthOfBytes(using: .utf8)))
            return Termplex.keyboardShortcut(for: trigger)
        }
#endif

        // MARK: - Configuration Values

        /// For all of the configuration values below, see the associated Termplex documentation for
        ...
        var bellFeatures: BellFeatures {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .init() }
            return .init(rawValue: v)
        }

        var bellAudioPath: ConfigPath? {
            guard let config = self.config else { return nil }
            var v = termplex_config_path_s()
            let key = "bell-audio-path"
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return nil }
        ...
        var bellAudioVolume: Float {
            ...
            _ = termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8)))
            return Float(v)
        }
        ...
        var notifyOnCommandFinish: NotifyOnCommandFinish {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .never }
        ...
        var notifyOnCommandFinishAction: NotifyOnCommandFinishAction {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return defaultValue }
            return .init(rawValue: v)
        }
        ...
        var splitPreserveZoom: SplitPreserveZoom {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.count)) else { return .init() }
            return .init(rawValue: v)
        }
        ...
        var windowSaveState: String {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return "" }
        ...
        var windowPositionX: Int16? {
            ...
            return termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) ? v : nil
        }
        ...
        var windowFullscreen: Bool {
            ...
            var v: UnsafePointer<Int8>?
            let key = "fullscreen"
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return false }
        ...
        var macosCustomIcon: String {
            #if os(macOS)
            let defaultValue = NSString("~/.config/termplex/Termplex.icns").expandingTildeInPath
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>?
        ...
        var backgroundColor: Color {
            var color: termplex_config_color_s = .init()
            let bg_key = "background"
            if !termplex_config_get(config, &color, bg_key, UInt(bg_key.lengthOfBytes(using: .utf8))) {
#if os(macOS)
...
#error("unsupported")
        ...
        var unfocusedSplitOpacity: Double {
            ...
            _ = termplex_config_get(config, &opacity, key, UInt(key.lengthOfBytes(using: .utf8)))
            return 1 - opacity
        }
        ...
        var unfocusedSplitFill: Color {
            ...
            if !termplex_config_get(config, &color, key, UInt(key.lengthOfBytes(using: .utf8))) {
                let bg_key = "background"
                _ = termplex_config_get(config, &color, bg_key, UInt(bg_key.lengthOfBytes(using: .utf8)))
            }
        ...
        var quickTerminalPosition: QuickTerminalPosition {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .top }
        ...
        var quickTerminalScreen: QuickTerminalScreen {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .main }
            guard let ptr = v else { return .main }
            let str = String(cString: ptr)
            return QuickTerminalScreen(fromTermplexConfig: str) ?? .main
        }
        ...
        var quickTerminalSpaceBehavior: QuickTerminalSpaceBehavior {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .move }
            guard let ptr = v else { return .move }
            let str = String(cString: ptr)
            return QuickTerminalSpaceBehavior(fromTermplexConfig: str) ?? .move
        }

        var quickTerminalSize: QuickTerminalSize {
            guard let config = self.config else { return QuickTerminalSize() }
            var v = termplex_config_quick_terminal_size_s()
            let key = "quick-terminal-size"
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return QuickTerminalSize() }
        ...
        var resizeOverlay: ResizeOverlay {
            ...
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return .after_first }
        ...
        var commandPaletteEntries: [Termplex.Command] {
            guard let config = self.config else { return [] }
            var v: termplex_config_command_list_s = .init()
            let key = "command-palette-entry"
            guard termplex_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return [] }
            guard v.len > 0 else { return [] }
            let buffer = UnsafeBufferPointer(start: v.commands, count: v.len)
            return buffer.map { Termplex.Command(cValue: $0) }
        }
