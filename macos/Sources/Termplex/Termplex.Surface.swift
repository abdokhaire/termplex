...
extension Termplex {
    /// Represents a single surface within Termplex.
    ///
    /// NOTE(mitchellh): This is a work-in-progress class as part of a general refactor
    /// of our Termplex data model. At the time of writing there's still a ton of surface
    /// functionality that is not encapsulated in this class. It is planned to migrate that
    /// all over.
    ///
    /// Wraps a `termplex_surface_t`
    final class Surface: Sendable {
        private let surface: termplex_surface_t
        ...
        var unsafeCValue: termplex_surface_t {
            surface
        }

        /// Initialize from the C structure.
        init(cSurface: termplex_surface_t) {
            self.surface = cSurface
        }

        deinit {
            // deinit is not guaranteed to happen on the main actor and our API
            // calls into libtermplex must happen there so we capture the surface
        ...
        func sendKeyEvent(_ event: Input.KeyEvent) {
            event.withCValue { cEvent in
                termplex_surface_key(surface, cEvent)
            }
        }
        ...
        @MainActor
        func keyIsBinding(_ event: termplex_input_key_s) -> Input.BindingFlags? {
            var flags = termplex_binding_flags_e(0)
            guard termplex_surface_key_is_binding(surface, event, &flags) else { return nil }
            return Input.BindingFlags(cFlags: flags)
        }

        /// See `keyIsBinding(_ event: termplex_input_key_s)`.
        @MainActor
        ...
        var mouseCaptured: Bool {
            termplex_surface_mouse_captured(surface)
        }
