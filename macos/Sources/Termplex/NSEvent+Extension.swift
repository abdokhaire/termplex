...
extension NSEvent {
    /// Create a Termplex key event for a given keyboard action.
    ///
    ...
    func termplexKeyEvent(
        ...
        key_ev.mods = Termplex.termplexMods(modifierFlags)
        key_ev.consumed_mods = Termplex.termplexMods(
    ...
    /// Returns the text to set for a key event for Termplex.
    ///
    /// This namely contains logic to avoid control characters, since we handle control character
    /// mapping manually within Termplex.
    var termplexCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within Termplex's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to Termplex.
