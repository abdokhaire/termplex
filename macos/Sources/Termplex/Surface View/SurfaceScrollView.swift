...
/// Wraps a Termplex surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual Termplex renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: Termplex.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: Termplex.SurfaceView) {
        ...
        // so that our primary Termplex renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from Termplex
        observers.append(NotificationCenter.default.addObserver(
            forName: .termplexDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? Termplex.Action.Scrollbar else {
            return
        }
