...
extension Termplex {
    ...
    class SurfaceView: OSView, ObservableObject, Codable, Identifiable {
                ...
                if let searchState {
                    ...
                    searchNeedleCancellable = searchState.$needle
                        ...
                        .sink { [weak self] needle in
                            ...
                            termplex_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8)))
                        }
        ...
        @Published var surfaceSize: termplex_surface_size_s?
        ...
        // detected with the set_password_input_cb on the Termplex state.
        ...
        var needsConfirmQuit: Bool {
            ...
            return termplex_surface_needs_confirm_quit(surface)
        }

        // Returns true if the process in this surface has exited.
        var processExited: Bool {
            guard let surface = self.surface else { return true }
            return termplex_surface_process_exited(surface)
        }

        // Returns the inspector instance for this surface, or nil if the
        // surface has been closed or no inspector is active.
        var inspector: Termplex.Inspector? {
            guard let surface = self.surface else { return nil }
            guard let cInspector = termplex_surface_inspector(surface) else { return nil }
            return Termplex.Inspector(cInspector: cInspector)
        }
        ...
        @Published var inspectorVisible: Bool = false {
            didSet {
                if oldValue && !inspectorVisible {
                    guard let surface = self.surface else { return }
                    termplex_inspector_free(surface)
                }
            }
        }
        ...
        private(set) var surfaceModel: Termplex.Surface?

        /// Returns the underlying C value for the surface. See "note" on surfaceModel.
        var surface: termplex_surface_t? {
        ...
        var scrollbar: Termplex.Action.Scrollbar?
        ...
        init(_ app: termplex_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            self.markedText = NSMutableAttributedString()
            self.id = uuid ?? .init()

            // Our initial config always is our application wide config.
            if let appDelegate = NSApplication.shared.delegate as? AppDelegate {
                self.derivedConfig = DerivedConfig(appDelegate.termplex.config)
            } else {
            ...
            cachedScreenContents = .init(duration: .milliseconds(500)) { [weak self] in
                ...
                guard let surface = self.surface else { return "" }
                var text = termplex_text_s()
                let sel = termplex_selection_s(
                    top_left: termplex_point_s(
                        tag: TERMPLEX_POINT_SCREEN,
                        coord: TERMPLEX_POINT_COORD_TOP_LEFT,
                        x: 0,
                        y: 0),
                    bottom_right: termplex_point_s(
                        tag: TERMPLEX_POINT_SCREEN,
                        coord: TERMPLEX_POINT_COORD_BOTTOM_RIGHT,
                        x: 0,
                        y: 0),
                    rectangle: false)
                guard termplex_surface_read_text(surface, sel, &text) else { return "" }
                defer { termplex_surface_free_text(surface, &text) }
            ...
            center.addObserver(
                ...
                name: Termplex.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(termplexDidContinueKeySequence),
                name: Termplex.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(termplexDidEndKeySequence),
                name: Termplex.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(termplexDidChangeKeyTable),
                name: Termplex.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
            ...
            center.addObserver(
                self,
                selector: #selector(termplexColorDidChange(_:)),
            ...
            center.addObserver(
                self,
                selector: #selector(termplexDidChangeReadonly(_:)),
            ...
            let surface = surface_cfg.withCValue(view: self) { surface_cfg_c in
                termplex_surface_new(app, &surface_cfg_c)
            }
            guard let surface = surface else {
                self.error = Termplex.Error.apiFailed
                return
            }
            self.surfaceModel = Termplex.Surface(cSurface: surface)
        ...
        func focusDidChange(_ focused: Bool) {
            ...
            // Notify libtermplex
            termplex_surface_set_focus(surface, focused)
        ...
        func sizeDidChange(_ size: CGSize) {
            // Termplex wants to know the actual framebuffer size... It is very important
        ...
        private func setSurfaceSize(width: UInt32, height: UInt32) {
            ...
            termplex_surface_set_size(surface, width, height)

            // Update our cached size metrics
            let size = termplex_surface_size(surface)
        ...
        func setCursorShape(_ shape: termplex_action_mouse_shape_e) {
            switch shape {
            case TERMPLEX_MOUSE_SHAPE_DEFAULT:
                pointerStyle = .default

            case TERMPLEX_MOUSE_SHAPE_TEXT:
                pointerStyle = .horizontalText

            case TERMPLEX_MOUSE_SHAPE_GRAB:
                pointerStyle = .grabIdle

            case TERMPLEX_MOUSE_SHAPE_GRABBING:
                pointerStyle = .grabActive

            case TERMPLEX_MOUSE_SHAPE_POINTER:
                pointerStyle = .link

            case TERMPLEX_MOUSE_SHAPE_W_RESIZE:
                pointerStyle = .resizeLeft

            case TERMPLEX_MOUSE_SHAPE_E_RESIZE:
                pointerStyle = .resizeRight

            case TERMPLEX_MOUSE_SHAPE_N_RESIZE:
                pointerStyle = .resizeUp

            case TERMPLEX_MOUSE_SHAPE_S_RESIZE:
                pointerStyle = .resizeDown

            case TERMPLEX_MOUSE_SHAPE_NS_RESIZE:
                pointerStyle = .resizeUpDown

            case TERMPLEX_MOUSE_SHAPE_EW_RESIZE:
                pointerStyle = .resizeLeftRight

            case TERMPLEX_MOUSE_SHAPE_VERTICAL_TEXT:
                pointerStyle = .verticalText

            case TERMPLEX_MOUSE_SHAPE_CONTEXT_MENU:
                pointerStyle = .contextMenu

            case TERMPLEX_MOUSE_SHAPE_CROSSHAIR:
                pointerStyle = .crosshair

            case TERMPLEX_MOUSE_SHAPE_NOT_ALLOWED:
        ...
        @objc private func onUpdateRendererHealth(notification: SwiftUI.Notification) {
            ...
            guard let health = healthAny as? termplex_action_renderer_health_e else { return }
            DispatchQueue.main.async { [weak self] in
                self?.healthy = health == TERMPLEX_RENDERER_HEALTH_HEALTHY
            }
        }

        @objc private func termplexDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[Termplex.Notification.KeySequenceKey] else { return }
        ...
        @objc private func termplexDidEndKeySequence(notification: SwiftUI.Notification) {
            DispatchQueue.main.async { [weak self] in
                self?.keySequence = []
            }
        }

        @objc private func termplexDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[Termplex.Notification.KeyTableKey] as? Termplex.Action.KeyTable else { return }
        ...
        @objc private func termplexConfigDidChange(_ notification: SwiftUI.Notification) {
            // Get our managed configuration object out
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.TermplexConfigChangeKey
        ...
        @objc private func termplexColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.TermplexColorChangeKey
            ] as? Termplex.Action.ColorChange else { return }
        ...
        @objc private func termplexBellDidRing(_ notification: SwiftUI.Notification) {
            // Bell state goes to true
            bell = true
        }

        @objc private func termplexDidChangeReadonly(_ notification: SwiftUI.Notification) {
        ...
        @objc private func windowDidChangeScreen(notification: SwiftUI.Notification) {
            ...
            // When the window changes screens, we need to update libtermplex with the screen
            // ID. If vsync is enabled, this will be used with the CVDisplayLink to ensure
            // the proper refresh rate is going.
            termplex_surface_set_display_id(surface, screen.displayID ?? 0)
            ...
            // Issue: https://github.com/termplex-org/termplex/issues/2731
            DispatchQueue.main.async { [weak self] in
        ...
        override func viewDidChangeBackingProperties() {
            ...
            termplex_surface_set_content_scale(surface, xScale, yScale)
        ...
        override func mouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Termplex.termplexMods(event.modifierFlags)
            termplex_surface_mouse_button(surface, TERMPLEX_MOUSE_PRESS, TERMPLEX_MOUSE_LEFT, mods)
        }
        ...
        override func mouseUp(with event: NSEvent) {
            ...
            guard let surface = self.surface else { return }
            let mods = Termplex.termplexMods(event.modifierFlags)
            termplex_surface_mouse_button(surface, TERMPLEX_MOUSE_RELEASE, TERMPLEX_MOUSE_LEFT, mods)

            // Release pressure
            termplex_surface_mouse_pressure(surface, 0, 0)
        }

        override func otherMouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Termplex.termplexMods(event.modifierFlags)
            let button = Termplex.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            termplex_surface_mouse_button(surface, TERMPLEX_MOUSE_PRESS, button.cMouseButton, mods)
        }
        ...
        override func otherMouseUp(with event: NSEvent) {
            ...
            let mods = Termplex.termplexMods(event.modifierFlags)
            let button = Termplex.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            termplex_surface_mouse_button(surface, TERMPLEX_MOUSE_RELEASE, button.cMouseButton, mods)
        }
        ...
        override func rightMouseDown(with event: NSEvent) {
            ...
            let mods = Termplex.termplexMods(event.modifierFlags)
            if termplex_surface_mouse_button(
                surface,
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = Termplex.Input.MousePosEvent(
                x: pos.x,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify Termplex first. We do this because this will let Termplex handle
            // state setup that we'll need for later pressure handling (such as
            // QuickLook)
            termplex_surface_mouse_pressure(surface, UInt32(event.stage), Double(event.pressure))
            ...
            guard UserDefaults.termplex.bool(forKey: "com.apple.trackpad.forceClick") else { return }
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsTermplex = Termplex.eventModifierFlags(
                mods: termplex_surface_key_translation_mods(
                    surface,
                    Termplex.termplexMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsTermplex directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if translationModsTermplex.contains(flag) {
            ...
            let action = event.isARepeat ? TERMPLEX_ACTION_REPEAT : TERMPLEX_ACTION_PRESS
            ...
            if let list = keyTextAccumulator, list.count > 0 {
                ...
                _ = keyAction(
                    ...
                    event: event,
                    translationEvent: translationEvent,
                    text: translationEvent.termplexCharacters,
        ...
        override func keyUp(with event: NSEvent) {
            _ = keyAction(TERMPLEX_ACTION_RELEASE, event: event)
        }
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            let bindingFlags = surfaceModel.flatMap { surface in
                ...
                return (event.characters ?? "").withCString { ptr in
                    ...
                    return surface.keyIsBinding(termplexEvent)
                }
            }
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performTermplexBindingMenuKeyEquivalent(with: event) {
                        return true
                    }
                }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            case 0x39: mod = TERMPLEX_MODS_CAPS.rawValue
            case 0x38, 0x3C: mod = TERMPLEX_MODS_SHIFT.rawValue
            case 0x3B, 0x3E: mod = TERMPLEX_MODS_CTRL.rawValue
            case 0x3A, 0x3D: mod = TERMPLEX_MODS_ALT.rawValue
            case 0x37, 0x36: mod = TERMPLEX_MODS_SUPER.rawValue
            default: return
            }
            ...
            var action = TERMPLEX_ACTION_RELEASE
        ...
        private func keyAction(
            ...
            var key_ev = event.termplexKeyEvent(action, translationMods: translationEvent?.modifierFlags)
            key_ev.composing = composing

            // For text, we only encode UTF8 if we don't have a single control
            // character. Control characters are encoded by Termplex itself.
            // Without this, `ctrl+enter` does the wrong thing.
            if let text, text.count > 0,
               let codepoint = text.utf8.first, codepoint >= 0x20 {
                return text.withCString { ptr in
                    key_ev.text = ptr
                    return termplex_surface_key(surface, key_ev)
                }
            } else {
                return termplex_surface_key(surface, key_ev)
            }
        ...
        override func quickLook(with event: NSEvent) {
            ...
            var text = termplex_text_s()
            guard termplex_surface_quicklook_word(surface, &text) else { return super.quickLook(with: event) }
            defer { termplex_surface_free_text(surface, &text) }
            ...
            if let fontRaw = termplex_surface_quicklook_font(surface) {
                // Memory management here is wonky: termplex_surface_quicklook_font
            ...
            // Termplex coordinate system is top-left, convert to bottom-left for AppKit
        ...
        @IBAction func splitRight(_ sender: Any) {
            guard let surface = self.surface else { return }
            termplex_surface_split(surface, TERMPLEX_SPLIT_DIRECTION_RIGHT)
        }

        @IBAction func splitLeft(_ sender: Any) {
            guard let surface = self.surface else { return }
            termplex_surface_split(surface, TERMPLEX_SPLIT_DIRECTION_LEFT)
        }

        @IBAction func splitDown(_ sender: Any) {
            guard let surface = self.surface else { return }
            termplex_surface_split(surface, TERMPLEX_SPLIT_DIRECTION_DOWN)
        }

        @IBAction func splitUp(_ sender: Any) {
            guard let surface = self.surface else { return }
            termplex_surface_split(surface, TERMPLEX_SPLIT_DIRECTION_UP)
        }
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            content.categoryIdentifier = Termplex.userNotificationCategory
            content.userInfo = [
        ...
        struct DerivedConfig {
            ...
            let backgroundBlur: Termplex.Config.BackgroundBlur
            let macosWindowShadow: Bool
            let windowTitleFontFamily: String?
            let windowAppearance: NSAppearance?
            let scrollbar: Termplex.Config.Scrollbar
            ...
            init(_ config: Termplex.Config) {
                ...
                self.windowAppearance = .init(termplexConfig: config)
...
extension Termplex.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the Termplex API. There is a race condition between getting the
        ...
        var text = termplex_text_s()
        guard termplex_surface_read_selection(surface, &text) else { return NSRange() }
        defer { termplex_surface_free_text(surface, &text) }
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // Termplex.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
        ...
        var text = termplex_text_s()
        guard termplex_surface_read_selection(surface, &text) else { return nil }
        defer { termplex_surface_free_text(surface, &text) }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // Termplex will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        if range.length > 0 && range != self.selectedRange() {
            // QuickLook
            var text = termplex_text_s()
            if termplex_surface_read_selection(surface, &text) {
                ...
                // Free our text
                termplex_surface_free_text(surface, &text)
            } else {
                termplex_surface_ime_point(surface, &x, &y, &width, &height)
            }
        } else {
            termplex_surface_ime_point(surface, &x, &y, &width, &height)
        }
        ...
        // Termplex coordinates are in top-left (0, 0) so we have to convert to
    ...
    /// Sync the preedit state based on the markedText value to libtermplex
...
extension Termplex.SurfaceView: NSServicesMenuRequestor {
    override func validRequestor(
        ...
        if (returnType == nil || receivable.contains(returnType!)) &&
            ...
            // performance hits because `termplex_surface_has_selection` isn't free.
            if let sendType, sendableRequiresSelection.contains(sendType) {
                if surface == nil || !termplex_surface_has_selection(surface) {
    ...
    func writeSelection(
        ...
        // Read the selection
        var text = termplex_text_s()
        guard termplex_surface_read_selection(surface, &text) else { return false }
        defer { termplex_surface_free_text(surface, &text) }
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        ...
        str.withCString { ptr in
            ...
            termplex_surface_text(surface, ptr, UInt(len - 1))
        }
...
extension Termplex.SurfaceView: NSMenuItemValidation {
    func validateMenuItem(_ item: NSMenuItem) -> Bool {
        switch item.action {
        case #selector(pasteSelection):
            let pb = NSPasteboard.termplexSelection
...
extension Termplex.SurfaceView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        if let url = pb.string(forType: .URL) {
            ...
            content = Termplex.Shell.escape(url)
           ...
           urls.count > 0 {
            ...
            content = urls
                .map { Termplex.Shell.escape($0.path) }
                .joined(separator: " ")
...
extension Termplex.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment Termplex core to
    /// expose that.
    ///
