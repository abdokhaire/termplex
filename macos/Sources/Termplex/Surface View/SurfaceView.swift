...
extension Termplex {
    ...
    struct Terminal: View {
        @EnvironmentObject private var termplex: Termplex.App

        var body: some View {
            if let app = self.termplex.app {
    ...
    /// Yields a SurfaceView for a termplex app that can then be used however you want.
    struct SurfaceForApp<Content: View>: View {
        let content: ((SurfaceView) -> Content)

        @StateObject private var surfaceView: SurfaceView

        init(_ app: termplex_app_t, @ViewBuilder content: @escaping ((SurfaceView) -> Content)) {
    ...
    struct SurfaceWrapper: View {
        ...
        @EnvironmentObject private var termplex: Termplex.App
        @Environment(\.termplexLastFocusedSurface) private var lastFocusedSurface
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    SurfaceRepresentable(view: surfaceView, size: geo.size)
                        ...
                        .focusedValue(\.termplexSurfacePwd, surfaceView.pwd)
                        .focusedValue(\.termplexSurfaceView, surfaceView)
                        .focusedValue(\.termplexSurfaceCellSize, surfaceView.cellSize)
                    #if canImport(AppKit)
                    ...
                    if let surfaceSize = surfaceView.surfaceSize {
                        SurfaceResizeOverlay(
                            geoSize: geo.size,
                            size: surfaceSize,
                            overlay: termplex.config.resizeOverlay,
                            position: termplex.config.resizeOverlayPosition,
                            duration: termplex.config.resizeOverlayDuration,
                            focusInstant: surfaceView.focusInstant)

                    }
                }
                .termplexSurfaceView(surfaceView)
                ...
                if termplex.config.secureInputIndication &&
                    secureInput.enabled &&
                    surfaceFocus &&
                    windowFocus {
                    SecureInputOverlay()
                ...
                if let searchState = surfaceView.searchState {
                    SurfaceSearchOverlay(
                        ...
                        onClose: {
#if canImport(AppKit)
                            Termplex.moveFocus(to: surfaceView)
#endif
                ...
                if termplex.config.bellFeatures.contains(.border) {
                ...
                if !surfaceView.healthy {
                    Rectangle().fill(termplex.config.backgroundColor)
                ...
                if isSplit && !isFocusedSurface {
                    let overlayOpacity = termplex.config.unfocusedSplitOpacity
                    if overlayOpacity > 0 {
                        Rectangle()
                            .fill(termplex.config.unfocusedSplitFill)
                            .allowsHitTesting(false)
    ...
    struct SurfaceResizeOverlay: View {
        let geoSize: CGSize
        let size: termplex_surface_size_s
        let overlay: Termplex.Config.ResizeOverlay
        let position: Termplex.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                ...
                .onReceive(NotificationCenter.default.publisher(for: .termplexSearchFocus)) { notification in
    ...
    /// A surface is terminology in Termplex for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libtermplex, usually from the Termplex configuration.
    struct SurfaceConfiguration {
        ...
        var context: termplex_surface_context_e = TERMPLEX_SURFACE_CONTEXT_WINDOW

        init() {}

        init(from config: termplex_surface_config_s) {
        ...
        /// Provides a C-compatible termplex configuration within a closure. The configuration
        /// and all its string pointers are only valid within the closure.
        func withCValue<T>(view: SurfaceView, _ body: (inout termplex_surface_config_s) throws -> T) rethrows -> T {
            var config = termplex_surface_config_new()
            config.userdata = Unmanaged.passUnretained(view).toOpaque()
#if os(macOS)
            config.platform_tag = TERMPLEX_PLATFORM_MACOS
            config.platform = termplex_platform_u(macos: termplex_platform_macos_s(
                nsview: Unmanaged.passUnretained(view).toOpaque()
            ))
            config.scale_factor = NSScreen.main!.backingScaleFactor
#elseif os(iOS)
            config.platform_tag = TERMPLEX_PLATFORM_IOS
            config.platform = termplex_platform_u(ios: termplex_platform_ios_s(
            ...
            // libtermplex APIs when a UIView is attached to a window/scene. TODO.
                    ...
                    return try initialInput.withCString { cInput in
                        ...
                        return try keys.withCStrings { keyCStrings in
                            return try values.withCStrings { valueCStrings in
                                // Create array of termplex_env_var_s
                                var envVars = [termplex_env_var_s]()
                                envVars.reserveCapacity(environmentVariables.count)
                                for i in 0..<environmentVariables.count {
                                    envVars.append(termplex_env_var_s(
                                        key: keyCStrings[i],
...
private struct TermplexSurfaceViewKey: EnvironmentKey {
    static let defaultValue: Termplex.SurfaceView? = nil
}

private struct TermplexLastFocusedSurfaceKey: EnvironmentKey {
    ...
    static let defaultValue: Weak<Termplex.SurfaceView>? = nil
}

extension EnvironmentValues {
    var termplexSurfaceView: Termplex.SurfaceView? {
        get { self[TermplexSurfaceViewKey.self] }
        set { self[TermplexSurfaceViewKey.self] = newValue }
    }

    var termplexLastFocusedSurface: Weak<Termplex.SurfaceView>? {
        get { self[TermplexLastFocusedSurfaceKey.self] }
        set { self[TermplexLastFocusedSurfaceKey.self] = newValue }
    }
}

extension View {
    func termplexSurfaceView(_ surfaceView: Termplex.SurfaceView?) -> some View {
        environment(\.termplexSurfaceView, surfaceView)
    }

    /// The most recently focused surface (can be currently focused if the surface is currently focused).
    func termplexLastFocusedSurface(_ surfaceView: Weak<Termplex.SurfaceView>?) -> some View {
        environment(\.termplexLastFocusedSurface, surfaceView)
    }
}
...
extension FocusedValues {
    var termplexSurfaceView: Termplex.SurfaceView? {
        get { self[FocusedTermplexSurface.self] }
        set { self[FocusedTermplexSurface.self] = newValue }
    }

    struct FocusedTermplexSurface: FocusedValueKey {
        typealias Value = Termplex.SurfaceView
    }

    var termplexSurfacePwd: String? {
        get { self[FocusedTermplexSurfacePwd.self] }
        set { self[FocusedTermplexSurfacePwd.self] = newValue }
    }

    struct FocusedTermplexSurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var termplexSurfaceCellSize: OSSize? {
        get { self[FocusedTermplexSurfaceCellSize.self] }
        set { self[FocusedTermplexSurfaceCellSize.self] = newValue }
    }

    struct FocusedTermplexSurfaceCellSize: FocusedValueKey {
...
extension Termplex.SurfaceView {
    class SearchState: ObservableObject {
        ...
        init(from startSearch: Termplex.Action.StartSearch) {
