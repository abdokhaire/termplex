...
extension Termplex {
    ...
    struct InspectableSurface: View {
        ...
        var body: some View {
            ...
            ZStack {
                if !surfaceView.inspectorVisible {
                    ...
                    SplitView(.vertical, $split, dividerColor: termplex.config.splitDividerColor, left: {
                        SurfaceWrapper(surfaceView: surfaceView, isSplit: isSplit)
                    }, right: {
        ...
        private func onControlInspector(_ notification: SwiftUI.Notification) {
            ...
            guard let mode = modeAny as? termplex_action_inspector_e else { return }

            switch mode {
            case TERMPLEX_INSPECTOR_TOGGLE:
                surfaceView.inspectorVisible = !surfaceView.inspectorVisible

            case TERMPLEX_INSPECTOR_SHOW:
                surfaceView.inspectorVisible = true

            case TERMPLEX_INSPECTOR_HIDE:
    ...
    class InspectorView: MTKView, NSTextInputClient {
        ...
        private var inspector: Termplex.Inspector? {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            inspector.mouseButton(TERMPLEX_MOUSE_PRESS, button: TERMPLEX_MOUSE_LEFT, mods: mods)
        }

        override func mouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = Termplex.termplexMods(event.modifierFlags)
            inspector.mouseButton(TERMPLEX_MOUSE_RELEASE, button: TERMPLEX_MOUSE_LEFT, mods: mods)
        }

        override func rightMouseDown(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = Termplex.termplexMods(event.modifierFlags)
            inspector.mouseButton(TERMPLEX_MOUSE_PRESS, button: TERMPLEX_MOUSE_RIGHT, mods: mods)
        }

        override func rightMouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = Termplex.termplexMods(event.modifierFlags)
            inspector.mouseButton(TERMPLEX_MOUSE_RELEASE, button: TERMPLEX_MOUSE_RIGHT, mods: mods)
        }
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            var momentum: termplex_input_mouse_momentum_e = TERMPLEX_MOUSE_MOMENTUM_NONE
            switch event.momentumPhase {
            case .began:
                momentum = TERMPLEX_MOUSE_MOMENTUM_BEGAN
            case .stationary:
                momentum = TERMPLEX_MOUSE_MOMENTUM_STATIONARY
            case .changed:
                momentum = TERMPLEX_MOUSE_MOMENTUM_CHANGED
            case .ended:
                momentum = TERMPLEX_MOUSE_MOMENTUM_ENDED
            case .cancelled:
                momentum = TERMPLEX_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin:
                momentum = TERMPLEX_MOUSE_MOMENTUM_MAY_BEGIN
            default:
                break
            }
        ...
        override func keyUp(with event: NSEvent) {
            keyAction(TERMPLEX_ACTION_RELEASE, event: event)
        }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            if mods.rawValue & mod != 0 { action = TERMPLEX_ACTION_PRESS }

            keyAction(action, event: event)
        }

        private func keyAction(_ action: termplex_input_action_e, event: NSEvent) {
            guard let inspector = self.inspector else { return }
            guard let key = Termplex.Input.Key(keyCode: event.keyCode) else { return }
            let mods = Termplex.termplexMods(event.modifierFlags)
