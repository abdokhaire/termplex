...
extension Termplex.SurfaceView: Transferable {
    static var transferRepresentation: some TransferRepresentation {
        DataRepresentation(contentType: .termplexSurfaceId) { surface in
    ...
    static func find(uuid: UUID) -> Self? {
        #if canImport(AppKit)
        guard let del = NSApp.delegate as? Termplex.Delegate else { return nil }
        return del.termplexSurface(id: uuid) as? Self
...
extension UTType {
    ...
    static let termplexSurfaceId = UTType(exportedAs: "com.mitchellh.termplexSurfaceId")
}

#if canImport(AppKit)
extension NSPasteboard.PasteboardType {
    /// Pasteboard type for dragging surface IDs.
    static let termplexSurfaceId = NSPasteboard.PasteboardType(UTType.termplexSurfaceId.identifier)
}
#endif
