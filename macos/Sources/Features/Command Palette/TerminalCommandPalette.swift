...
struct TerminalCommandPaletteView: View {
    ...
    @ObservedObject var termplexConfig: Termplex.Config
                ...
                GeometryReader { geometry in
                    VStack {
                        ...
                        CommandPaletteView(
                            ...
                            backgroundColor: termplexConfig.backgroundColor,
    ...
    private var updateOptions: [CommandOption] {
        ...
        if case .updateAvailable = updateViewModel.state {
            title = "Update Termplex and Restart"
        } else {
    ...
    private var terminalOptions: [CommandOption] {
        ...
        return appDelegate.termplex.config.commandPaletteEntries
            .filter(\.isSupported)
            .map { c in
                let symbols = appDelegate.termplex.config.keyboardShortcut(for: c.action)?.keyList
