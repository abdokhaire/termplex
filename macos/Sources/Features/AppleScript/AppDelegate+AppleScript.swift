...
// Application-level Cocoa scripting hooks for the Termplex AppleScript dictionary.
//
...
// the runtime object behind the `application` class in `Termplex.sdef`.
...
extension NSApplication {
    ...
    /// the frontmost logical Termplex window.
    @objc(frontWindow)
...
extension NSApplication {
    ...
    func handleNewSurfaceConfigurationScriptCommand(_ command: NSScriptCommand) -> NSDictionary? {
        ...
        do {
            let configuration = try Termplex.SurfaceConfiguration(
    ...
    func handleNewWindowScriptCommand(_ command: NSScriptCommand) -> ScriptWindow? {
        ...
        guard let appDelegate = delegate as? AppDelegate else {
            ...
            command.scriptErrorString = "Termplex app delegate is unavailable."
            return nil
        }
...
@MainActor
extension NSApplication {
    /// Whether Termplex should currently accept AppleScript interactions.
    var isAppleScriptEnabled: Bool {
        guard let appDelegate = delegate as? AppDelegate else { return true }
        return appDelegate.termplex.config.macosAppleScript
    }
    ...
    fileprivate var allSurfaceViews: [Termplex.SurfaceView] {
