...
/// Handler for the `send mouse button` AppleScript command defined in `Termplex.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="TermplexScriptMouseButtonCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(TermplexScriptMouseButtonCommand)
final class ScriptMouseButtonCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let action: Termplex.Input.MouseState
...
/// Four-character codes matching the `mouse button` enumeration in `Termplex.sdef`.
private enum ScriptMouseButtonValue {
    ...
    var termplexButton: Termplex.Input.MouseButton {
        switch self {
        case .left: .left
        case .right: .right
        case .middle: .middle
