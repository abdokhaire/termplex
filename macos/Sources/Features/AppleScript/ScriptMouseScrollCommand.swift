...
/// Handler for the `send mouse scroll` AppleScript command defined in `Termplex.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="TermplexScriptMouseScrollCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(TermplexScriptMouseScrollCommand)
final class ScriptMouseScrollCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let momentum: Termplex.Input.Momentum
