...
protocol TerminalRestorable: Codable {
    ...
    var baseConfig: Termplex.SurfaceConfiguration? { get }
}
...
extension TerminalRestorable {
    ...
    var baseConfig: Termplex.SurfaceConfiguration? { nil }
...
class TerminalWindowRestoration: NSObject, NSWindowRestoration {
    static func restoreWindow(
        ...
        // no matter what. Note its safe to use "termplex.config" directly here
        // because window restoration is only ever invoked on app start so we
        // don't have to deal with config reloads.
        if appDelegate.termplex.config.windowSaveState == "never" {
        ...
        // can be found for events from libtermplex. This uses the low-level
        ...
        if let focusedStr = state.focusedSurface {
            var foundView: Termplex.SurfaceView?
    ...
    private static func restoreFocus(to: Termplex.SurfaceView, inWindow: NSWindow, attempts: Int = 0) {
