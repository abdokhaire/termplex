...
class TerminalWindow: NSWindow {
    ...
    override func awakeFromNib() {
        ...
        // Get our saved level
        level = UserDefaults.termplex.value(forKey: Self.defaultLevelKey) as? NSWindow.Level ?? .normal
    }
    ...
    /// added.
    static let tabBarIdentifier: NSUserInterfaceItemIdentifier = .init("_termplexTabBar")
    ...
    func syncAppearance(_ surfaceConfig: Termplex.SurfaceView.DerivedConfig) {
        ...
        if !styleMask.contains(.fullScreen) &&
            ...
            if !surfaceConfig.backgroundBlur.isGlassStyle, let appDelegate = NSApp.delegate as? AppDelegate {
                termplex_set_window_background_blur(
                    appDelegate.termplex.app,
    ...
    var preferredBackgroundColor: NSColor? {
        if let terminalController, !terminalController.surfaceTree.isEmpty {
            let surface: Termplex.SurfaceView?
...
extension TerminalWindow {
    private static let closeTabsOnRightMenuItemIdentifier = NSUserInterfaceItemIdentifier("com.mitchellh.termplex.closeTabsOnTheRightMenuItem")
    private static let changeTitleMenuItemIdentifier = NSUserInterfaceItemIdentifier("com.mitchellh.termplex.changeTitleMenuItem")
    private static let tabColorSeparatorIdentifier = NSUserInterfaceItemIdentifier("com.mitchellh.termplex.tabColorSeparator")

    private static let tabColorPaletteIdentifier = NSUserInterfaceItemIdentifier("com.mitchellh.termplex.tabColorPalette")
