...
struct ColorizedTermplexIcon {
    ...
    let frame: Termplex.MacOSIconFrame

    /// Make a custom colorized termplex icon.
...
extension ColorizedTermplexIcon: Codable {
    ...
    init(from decoder: any Decoder) throws {
        ...
        guard version == 0 || version == CodingKeys.currentVersion else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    ...
                    debugDescription: "Unsupported ColorizedTermplexIcon version: \(version)"
                )
            )
        }
        ...
        let frame = try container.decode(Termplex.MacOSIconFrame.self, forKey: .frame)
...
extension ColorizedTermplexIcon: Equatable {
