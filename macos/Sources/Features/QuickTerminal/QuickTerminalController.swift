...
class QuickTerminalController: BaseTerminalController {
    ...
    /// The previously running application when the terminal is shown. This is NEVER Termplex.
    ...
    init(_ termplex: Termplex.App,
        ...
        super.init(termplex, baseConfig: base, surfaceTree: .init())
    ...
    override func focusSurface(_ view: Termplex.SurfaceView) {
        if visible {
        ...
        if surfaceTree.isEmpty,
           let termplex_app = termplex.app {
            if let tree = restorationState?.surfaceTree, !tree.isEmpty {
                ...
                config.environmentVariables["TERMPLEX_QUICK_TERMINAL"] = "1"

                let view = Termplex.SurfaceView(termplex_app, baseConfig: config)
        ...
        NSAnimationContext.runAnimationGroup({ context in
            ...
            DispatchQueue.main.async {
                ...
                if !NSApp.isActive {
                    ...
                    // shown on a screen that has no other Termplex windows, it takes
                    // a few (variable) event loop ticks until we can actually focus it.
                    // https://github.com/termplex-org/termplex/issues/2409
                    //
    ...
    override func syncAppearance() {
        ...
        if !isBackgroundOpaque && (self.derivedConfig.backgroundOpacity < 1 || derivedConfig.backgroundBlur.isGlassStyle) {
            ...
            if !derivedConfig.backgroundBlur.isGlassStyle {
                termplex_set_window_background_blur(termplex.app, Unmanaged.passUnretained(window).toOpaque())
            }
        } else {
        ...
        terminalViewContainer?.termplexConfigDidChange(termplex.config, preferredBackgroundColor: nil)
    }
    ...
    @objc private func termplexConfigDidChange(_ notification: Notification) {
        ...
        terminalViewContainer?.termplexConfigDidChange(config, preferredBackgroundColor: nil)
    }
