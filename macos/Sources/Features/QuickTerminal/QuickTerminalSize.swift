...
/// Represents the Termplex `quick-terminal-size` configuration. See the documentation for
...
struct QuickTerminalSize {
    ...
    init(from cStruct: termplex_config_quick_terminal_size_s) {
    ...
    enum Size {
        ...
        init?(from cStruct: termplex_quick_terminal_size_s) {
            switch cStruct.tag {
            case TERMPLEX_QUICK_TERMINAL_SIZE_NONE:
                return nil
            case TERMPLEX_QUICK_TERMINAL_SIZE_PERCENTAGE:
                self = .percentage(cStruct.value.percentage)
            case TERMPLEX_QUICK_TERMINAL_SIZE_PIXELS:
