...
/// Requests permission for Shortcuts app to interact with Termplex
///
/// This function displays a permission dialog asking the user to allow Shortcuts
/// to interact with Termplex. The permission is automatically cached for 10 minutes
...
/// permission for one intent allows all Termplex intents to execute without additional
    ...
    await withCheckedContinuation { continuation in
        Task { @MainActor in
            if let delegate = NSApp.delegate as? AppDelegate {
                switch delegate.termplex.config.macosShortcuts {
                case .allow:
            ...
            PermissionRequest.show(
                "com.mitchellh.termplex.shortcutsPermission",
                message: "Allow Shortcuts to interact with Termplex?",
