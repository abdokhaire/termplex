...
struct CommandEntity: AppEntity {
    ...
    init(_ command: Termplex.Command, for terminal: TerminalEntity) {
...
struct CommandQuery: EntityQuery {
    ...
    func entities(for identifiers: [CommandEntity.ID]) async throws -> [CommandEntity] {
        ...
        let commands = appDelegate.termplex.config.commandPaletteEntries
    ...
    func suggestedEntities() async throws -> [CommandEntity] {
        ...
        return appDelegate.termplex.config.commandPaletteEntries.map { CommandEntity($0, for: terminal) }
    }
}
