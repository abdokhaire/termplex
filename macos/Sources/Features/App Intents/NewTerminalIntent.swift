...
struct NewTerminalIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<TerminalEntity?> {
        ...
        guard let appDelegate = NSApp.delegate as? AppDelegate else {
            throw TermplexIntentError.appUnavailable
        }
        let termplex = appDelegate.termplex
        ...
        if let command {
            config.initialInput = "\(Termplex.Shell.quote(command)); exit\n"
        }
        ...
        let parent: Termplex.SurfaceView?
