...
private func cgEventFlagsChangedHandler(
    ...
    // We need an app delegate to get the Termplex app instance
    guard let appDelegate = NSApplication.shared.delegate as? AppDelegate else { return result }
    guard let termplex = appDelegate.termplex.app else { return result }
    ...
    let key_ev = event.termplexKeyEvent(TERMPLEX_ACTION_PRESS)
    if termplex_app_key(termplex, key_ev) {
