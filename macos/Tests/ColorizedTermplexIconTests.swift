...
struct ColorizedTermplexIconTests {
    private func makeIcon(
        ...
        frame: Termplex.MacOSIconFrame = .aluminum
    ) -> ColorizedTermplexIcon {
    ...
    @Test func codableRoundTripPreservesIcon() throws {
        ...
        let decoded = try JSONDecoder().decode(ColorizedTermplexIcon.self, from: data)
    ...
    @Test func decodingUnsupportedVersionThrowsDataCorrupted() {
        ...
        do {
            _ = try JSONDecoder().decode(ColorizedTermplexIcon.self, from: data)
            Issue.record("Expected decode to fail for unsupported version")
        } catch let DecodingError.dataCorrupted(context) {
            #expect(context.debugDescription.contains("Unsupported ColorizedTermplexIcon version"))
        } catch {
