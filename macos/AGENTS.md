...
- If code outside of `macos/` directory is modified, use
  ...
  the underlying Termplex library.
- Use `macos/build.nu` to build the macOS app, do not use `zig build`
  (except to build the underlying library as mentioned above).
  - Build: `macos/build.nu [--scheme Termplex] [--configuration Debug] [--action build]`
  - Output: `macos/build/<configuration>/Termplex.app` (e.g. `macos/build/Debug/Termplex.app`)
- Run unit tests directly with `macos/build.nu --action test`

## AppleScript

- The AppleScript scripting definition is in `macos/Termplex.sdef`.
...
- In `macos/Termplex.sdef`, keep top-level definitions in this order:
  1. Classes
  2. Records
  3. Enums
  4. Commands
- Test AppleScript support:
  ...
  (2) Launch and activate the app via osascript using the absolute path
      ...
      `osascript -e 'tell application "<absolute path to build/Debug/Termplex.app>" to activate'`
  ...
  (5) When done, quit via:
      `osascript -e 'tell application "<absolute path to build/Debug/Termplex.app>" to quit'`
