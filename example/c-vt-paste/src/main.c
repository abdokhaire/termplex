...
int main() {
  ...
  if (!termplex_paste_is_safe(unsafe_newline, strlen(unsafe_newline))) {
  ...
  if (!termplex_paste_is_safe(unsafe_escape, strlen(unsafe_escape))) {
    printf("Data with escape sequence is UNSAFE\n");
  }

  // Test empty data
  const char *empty_data = "";
  if (termplex_paste_is_safe(empty_data, 0)) {
