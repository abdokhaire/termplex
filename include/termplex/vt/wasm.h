 ...
 * @file wasm.h
 *
 * WebAssembly utility functions for libtermplex-vt.
 */

#ifndef TERMPLEX_VT_WASM_H
#define TERMPLEX_VT_WASM_H
 ...
 * **These are only available the libtermplex-vt wasm module.**
 *
 * Termplex relies on pointers to various types for ABI compatibility, and
 ...
 *
 * // Create key encoder
 * const encoderPtr = exports.termplex_wasm_alloc_opaque();
 * exports.termplex_key_encoder_new(null, encoderPtr);
 * const encoder = view.getUint32(encoder, true);
 *
 * // Configure encoder with Kitty protocol flags
 * const flagsPtr = exports.termplex_wasm_alloc_u8();
 * view.setUint8(flagsPtr, 0x1F);
 * exports.termplex_key_encoder_setopt(encoder, 5, flagsPtr);
 *
 * // Allocate output buffer and size pointer
 * const bufferSize = 32;
 * const bufPtr = exports.termplex_wasm_alloc_u8_array(bufferSize);
 * const writtenPtr = exports.termplex_wasm_alloc_usize();
 *
 * // Encode the key event
 * exports.termplex_key_encoder_encode(
 ...
 * to the libtermplex-vt Wasm module. In practice, this should be wrapped
 ...
 * types such as TermplexKeyEncoder, TermplexKeyEvent, etc.
 *
 * @return Pointer to allocated opaque pointer, or NULL if allocation failed
 * @ingroup wasm
 */
void** termplex_wasm_alloc_opaque(void);

/**
 * Free an opaque pointer allocated by termplex_wasm_alloc_opaque().
 *
 * @param ptr Pointer to free, or NULL (NULL is safely ignored)
 * @ingroup wasm
 */
void termplex_wasm_free_opaque(void **ptr);
 ...
 * @ingroup wasm
 */
uint8_t* termplex_wasm_alloc_u8_array(size_t len);

/**
 * Free an array allocated by termplex_wasm_alloc_u8_array().
 *
 ...
 * @ingroup wasm
 */
void termplex_wasm_free_u8_array(uint8_t *ptr, size_t len);
 ...
 * @ingroup wasm
 */
uint16_t* termplex_wasm_alloc_u16_array(size_t len);

/**
 * Free an array allocated by termplex_wasm_alloc_u16_array().
 *
 * @param ptr Pointer to the array to free, or NULL (NULL is safely ignored)
 * @param len Length of the array (must match the length passed to alloc)
 * @ingroup wasm
 */
void termplex_wasm_free_u16_array(uint16_t *ptr, size_t len);
 ...
 * @ingroup wasm
 */
uint8_t* termplex_wasm_alloc_u8(void);

/**
 * Free a uint8_t allocated by termplex_wasm_alloc_u8().
 *
 * @param ptr Pointer to free, or NULL (NULL is safely ignored)
 * @ingroup wasm
 */
void termplex_wasm_free_u8(uint8_t *ptr);
 ...
 * @ingroup wasm
 */
size_t* termplex_wasm_alloc_usize(void);

/**
 * Free a size_t allocated by termplex_wasm_alloc_usize().
 *
 * @param ptr Pointer to free, or NULL (NULL is safely ignored)
 * @ingroup wasm
 */
void termplex_wasm_free_usize(size_t *ptr);

/** @} */

#endif /* __wasm__ */

#endif /* TERMPLEX_VT_WASM_H */
