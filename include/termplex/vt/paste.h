...
#ifndef TERMPLEX_VT_PASTE_H
#define TERMPLEX_VT_PASTE_H
 ...
 * ## Basic Usage
 *
 * Use termplex_paste_is_safe() to check if paste data contains potentially
 ...
 * 
 *   if (termplex_paste_is_safe(safe_data, strlen(safe_data))) {
 *     printf("Safe to paste\n");
 *   }
 * 
 *   if (!termplex_paste_is_safe(unsafe_data, strlen(unsafe_data))) {
 ...
 */
bool termplex_paste_is_safe(const char* data, size_t len);
