 ...
 * @file allocator.h
 *
 * Memory management interface for libtermplex-vt.
 */

#ifndef TERMPLEX_VT_ALLOCATOR_H
#define TERMPLEX_VT_ALLOCATOR_H
 ...
 *
 * libtermplex-vt does require memory allocation for various operations,
 ...
 *
 * libtermplex-vt uses explicit memory allocation via an allocator
 * interface provided by TermplexAllocator. The interface is based on the
 ...
 * function that accepts one,** and libtermplex will use a default allocator.
 ...
 * 1. Implement the TermplexAllocatorVtable function pointers
 * 2. Create a TermplexAllocator struct with your vtable and context
 ...
 * Usage example:
 * @code
 * TermplexAllocator allocator = {
 *     .vtable = &my_allocator_vtable,
 *     .ctx = my_allocator_state
 * };
 * @endcode
 */
typedef struct TermplexAllocator {
    /**
     ...
     */
    const TermplexAllocatorVtable *vtable;
} TermplexAllocator;

/** @} */

#endif /* TERMPLEX_VT_ALLOCATOR_H */
