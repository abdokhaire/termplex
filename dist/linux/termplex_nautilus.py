...
DOMAIN = "com.termplex.app"
locale_dir = Path(__file__).absolute().parents[2] / "locale"
_ = gettext.translation(DOMAIN, locale_dir, fallback=True).gettext

def open_in_termplex_activated(_menu, paths):
    for path in paths:
        cmd = ['termplex', f'--working-directory={path}', '--gtk-single-instance=false']
...
def get_items_for_files(name, files):
    ...
    if paths:
        item = Nautilus.MenuItem(name=name, label=_('Open in Termplex'),
            icon='com.termplex.app')
        item.connect('activate', open_in_termplex_activated, paths)
        return [item]
    else:
        return []


class TermplexMenuProvider(GObject.GObject, Nautilus.MenuProvider):
    def get_file_items(self, files):
        return get_items_for_files('TermplexNautilus::open_in_termplex', files)

    def get_background_items(self, file):
        return get_items_for_files('TermplexNautilus::open_folder_in_termplex', [file])
