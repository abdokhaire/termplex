# Localizing Termplex: The Translators' Guide

First of all, thanks for helping us localize Termplex!
...
Full locale names are more complicated, but Termplex does not use all parts. [The
...
file called `com.termplex.app.pot`. **Do not edit this file.** The
template is generated automatically from Termplex's code and resources, and is
...
```console
$ msginit -i po/com.termplex.app.pot -l X -o "po/X.po"
```
...
Afterwards, you need to update the list of known locales within Termplex's
...
Simply run `zig build run`. Termplex uses your system language by default; if
...
On some desktop environments, such as KDE Plasma, Termplex uses server-side
decorations by default. This hides many strings from the UI, which is
undesirable when viewing your translations. You can force Termplex to use
...
Localization teams are represented as teams in the Termplex GitHub organization.
...
```diff
 # Localization
 /po/README_TRANSLATORS.md @termplex-org/localization
 /po/com.termplex.app.pot @termplex-org/localization
 /po/zh_CN.po @termplex-org/zh_CN
+/po/X.po @termplex-org/yy_ZZ
```
...
- **Make Termplex fit in with surrounding applications.**
  ...
  interface guidelines of each platform Termplex is available for, including the
