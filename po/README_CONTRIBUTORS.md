# Localizing Termplex: The Contributors' Guide

Termplex uses the `gettext` library/framework for localization, which has the
...
to localization efforts, as not all consumers of libtermplex would be interested
...
located under `po/com.termplex.app.pot`. **This file must stay in sync with
...
into binary `.mo` files, stored under `share/locale/<LOCALE>/LC_MESSAGES/com.termplex.app.mo`.
