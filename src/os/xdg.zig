...
test "cache directory paths" {
    ...
    // Test when XDG_CACHE_HOME is not set
    {
        ...
        // Test with subdir
        {
            ...
            defer alloc.free(cache_path);
            try testing.expectEqualStrings("/Users/test/.cache/termplex", cache_path);
        }
    }
}
...
test "fallback when xdg env empty" {
    ...
    const temp_home = "/tmp/termplex-test-home";
...
test "fallback when xdg env empty and subdir" {
    ...
    inline for (cases) |case| {
        ...
        const actual = try case.func(alloc, .{ .subdir = "termplex" });
        defer alloc.free(actual);
