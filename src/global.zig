...
/// Termplex can import this once and have access to the proper
/// backend.
pub const xev = @import("xev").Dynamic;

/// Global process state. This is initialized in main() for exe artifacts
/// and by termplex_init() for lib artifacts. This should ONLY be used by
...
pub const GlobalState = struct {
    ...
    pub fn init(self: *GlobalState) !void {
        ...
        // easy to set.
        if ((try internal_os.getenv(self.alloc, "TERMPLEX_LOG"))) |v| {
        ...
        std.log.info("termplex version={s}", .{build_config.version_string});
        std.log.info("termplex build optimize={s}", .{build_config.mode_string});
