...
/// `termplex.h` with the correct value. This should only ever be called inside a `test`
/// because the `termplex.h` module is only available then.
pub fn checkTermplexHEnum(comptime T: type, comptime prefix: []const u8) !void {
    ...
    std.testing.expect(set.count() == 0) catch |e| {
        ...
        while (it.next()) |v| {
            ...
            std.log.err("termplex.h is missing value for {s}{s}", .{ prefix, upper_string });
        }
        return e;
    };
}
