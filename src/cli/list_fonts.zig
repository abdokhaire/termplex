...
/// Termplex. This uses the exact same font discovery mechanism Termplex uses to
/// find fonts to use.
///
...
/// same priority order Termplex would use to pick a font.
///
/// Flags:
///
...
///     is identical to the `font-family` set of Termplex configuration values, so
