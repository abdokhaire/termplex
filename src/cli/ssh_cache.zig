...
/// Termplex automatically installs its terminfo on remote hosts. This command
...
///
/// Examples:
///   termplex +ssh-cache                          # List all cached hosts
///   termplex +ssh-cache --host=example.com       # Check if host is cached
///   termplex +ssh-cache --add=example.com        # Manually add host to cache
///   termplex +ssh-cache --add=user@example.com   # Add user@host combination
///   termplex +ssh-cache --remove=example.com     # Remove host from cache
///   termplex +ssh-cache --clear                  # Clear entire cache
///   termplex +ssh-cache --expire-days=30         # Set custom expiration period
...
pub fn runInner(
    ...
    const cache_path = try DiskCache.defaultPath(alloc, "termplex");
    ...
    if (opts.host) |host| {
        ...
        if (cached) {
            try stdout.print(
                "'{s}' has Termplex terminfo installed.\n",
                .{host},
            );
            return 0;
        } else {
            try stdout.print(
                "'{s}' does not have Termplex terminfo installed.\n",
                .{host},
            );
            return 1;
        }
