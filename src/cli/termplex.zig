...
pub const Action = enum {
    ...
    // Show which font face Termplex loads a codepoint from.
    @"show-face",
    ...
    // Use IPC to tell the running Termplex to open a new window.
    @"new-window",

    // Use IPC to tell the running Termplex to create a new split.
    @"new-split",

    pub fn detectSpecialCase(arg: []const u8) ?SpecialCase(Action) {
        // If we see a "-e" and we haven't seen a command yet, then
        // we are done looking for commands. This special case enables
        // `termplex -e termplex +command`. If we've seen a command we
        // still want to keep looking because
        // `termplex +command -e +command` is invalid.
