...
/// The `edit-config` command opens the Termplex configuration file in the
...
/// application menu, configured keybind, or by restarting Termplex. We
/// plan to auto-reload in the future, but Termplex isn't capable of
/// this yet.
///
/// The filepath opened is the default user-specific configuration
/// file, which is typically located at `$XDG_CONFIG_HOME/termplex/config.termplex`.
/// On macOS, this may also be located at
/// `~/Library/Application Support/com.termplex.app/config.termplex`.
...
fn runInner(alloc: Allocator, stderr: *std.Io.Writer) !u8 {
    ...
    if (comptime builtin.os.tag == .windows) {
        try stderr.print(
            \\The `termplex +edit-config` command is not supported on Windows.
    ...
    if (editor.len == 0) {
        try stderr.print(
            ...
            \\This environment variable is required to edit the Termplex configuration
    ...
    // limitation later but Termplex already heavily requires libc
