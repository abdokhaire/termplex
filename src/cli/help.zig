...
/// The `help` command shows general help about Termplex. Recognized as either
...
pub fn run(alloc: Allocator) !u8 {
    ...
    try stdout.writeAll(
        \\Usage: termplex [+action] [options]
        \\
        \\Run the Termplex terminal emulator or a specific helper action.
        \\
        \\If no `+action` is specified, run the Termplex terminal emulator.
        ...
        \\`termplex -e top` will run the `top` command inside the terminal.
        \\
        \\On macOS, launching the terminal emulator from the CLI is not
        \\supported and only actions are supported. Use `open -na Termplex.app`
        \\instead, or `open -na termplex.app --args --foo=bar --baz=quz` to pass
        \\arguments.
        \\
        \\Available actions:
        \\
