...
///
/// On all platforms, this is `${XDG_STATE_HOME}/termplex/ssh_cache`.
///
...
pub fn add(
    ...
    const result: AddResult = if (!gop.found_existing) add: {
        ...
        const terminfo_copy = try alloc.dupe(u8, "xterm-termplex");
...
test "disk cache default path" {
    ...
    const path = try DiskCache.defaultPath(alloc, "termplex");
