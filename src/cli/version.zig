...
/// The `version` command is used to display information about Termplex. Recognized as
...
pub fn run(alloc: Allocator) !u8 {
    ...
    if (tty) if (build_config.version.build) |commit_hash| {
        try stdout.print(
            "\x1b]8;;https://github.com/termplex-org/termplex/commit/{s}\x1b\\",
            .{commit_hash},
        );
    };
    try stdout.print("Termplex {s}\n\n", .{build_config.version_string});
    ...
    if (comptime build_config.app_runtime == .gtk) {
        ...
        // We say `libwayland` since it is possible to build Termplex without
