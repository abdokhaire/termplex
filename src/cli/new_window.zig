...
pub const Options = struct {
    ...
    /// If set, open up a new window in a custom instance of Termplex.
...
/// running instance of Termplex.
///
/// If the `--class` flag is not set, the `new-window` command will try and
/// connect to a running instance of Termplex based on what optimizations the
/// Termplex CLI was compiled with. Otherwise the `new-window` command will try
/// and contact a running Termplex instance that was configured with the same
/// `class` as was given on the command line.
///
/// All of the arguments after the `+new-window` argument (except for the
/// `--class` flag) will be sent to the remote Termplex instance and will be
...
/// relative to the current working directory that the `termplex +new-window`
...
/// directory that `termplex +new-window` is run from will be passed to Termplex.
///
/// GTK uses an application ID to identify instances of applications. If Termplex
/// is compiled with release optimizations, the default application ID will be
/// `com.termplex.app`. If Termplex is compiled with debug optimizations,
/// the default application ID will be `com.termplex.app-debug`.  The
...
/// or it will be ignored and Termplex will use the default as defined above.
///
/// On GTK, D-Bus activation must be properly configured. Termplex does not need
...
/// of Termplex if it is not already running. See the Termplex website for
...
///     Termplex. The class must be a valid GTK application ID.
///
///   * `--command`: The command to be executed in the first surface of the new window.
///
///   * `--working-directory=<directory>`: The working directory to pass to Termplex.
///
