...
/// The `show-face` command shows what font face Termplex will use to render a
...
/// Termplex uses to render a codepoint in a terminal session.
///
/// Flags:
///
...
fn runArgs(
    ...
    // Its possible to build Termplex without font discovery!
    if (comptime font.Discover == void) {
        try stderr.print(
            \\Termplex was built without a font discovery mechanism. This is a compile-time
            \\option. Please review how Termplex was built from source, contact the
...
fn lookup(
    ...
    const face = font_grid.resolver.collection.getFace(idx) catch |err| switch (err) {
        error.SpecialHasNoFace => {
            try stdout.print("U+{0X:0>2} « {0u} » is handled by Termplex's internal sprites.\n", .{cp});
            return null;
        },
        else => {
