//! The main entrypoint for the `termplex` application. This also serves
//! as the process initialization code for the `libtermplex` library.
...
const Termplex = @import("main_c.zig").Termplex;
...
pub fn main() !MainReturn {
        ...
        switch (@as(ErrSet, @errorCast(err))) {
            ...
            error.InvalidAction => try stderr.print(
                "Error: unknown CLI action specified. CLI actions are specified with\n" ++
                    "the '+' character.\n\n" ++
                    "All valid CLI actions can be listed with `termplex +help`\n",
                .{},
            ),
    ...
    if (comptime builtin.mode == .Debug) {
        ...
        std.log.warn("You should only use a debug build for developing Termplex.", .{});
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print("Usage: termplex +<action> [flags]\n\n", .{});
        try stdout.print(
            \\This is the Termplex helper CLI that accompanies the graphical Termplex app.
            \\To launch the terminal directly, please launch the graphical app
            \\(i.e. Termplex.app on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na Termplex.app`,
            \\or `open -na Termplex.app --args --foo=bar --baz=qux` to pass arguments.
            \\
...
fn logFn(
    ...
    //
    //   sudo log stream --level debug --predicate 'subsystem=="com.termplex.app"'
    //
...
pub const std_options: std.Options = .{
    ...
    // via TERMPLEX_LOG because our debug logs are very expensive to
