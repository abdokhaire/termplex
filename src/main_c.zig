// This is the main file for the C API. The C API is used to embed Termplex
...
// This is currently used only to embed termplex within a macOS app. However,
...
const main = @import("main_termplex.zig");
...
comptime {
    ...
    // functions unless we are building libtermplex.
    if (!builtin.is_test) {
...
comptime {
    ...
    // Any apprt-specific C API, mainly libtermplex for apprt.embedded.
...
/// Initialize termplex global state.
pub export fn termplex_init(argc: usize, argv: [*][*:0]u8) c_int {
    assert(builtin.link_libc);

    std.os.argv = argv[0..argc];
    state.init() catch |err| {
        std.log.err("failed to initialize termplex error={}", .{err});
        return 1;
    };
...
pub export fn termplex_cli_try_action() void {
...
/// Return metadata about Termplex, such as version, build mode, etc.
pub export fn termplex_info() Info {
    return .{
...
/// Translate a string maintained by libtermplex into the current
...
/// the function call.
///
/// This should only be used for singular strings maintained by Termplex.
pub export fn termplex_translate(msgid: [*:0]const u8) [*:0]const u8 {
    return internal_os.i18n._(msgid);
}

/// Free a string allocated by Termplex.
pub export fn termplex_string_free(str: String) void {
    str.deinit();
}

test "termplex_string_s empty string" {
...
test "termplex_string_s c string" {
    const testing = std.testing;
...
test "termplex_string_s zig string" {
    const testing = std.testing;
    state.alloc = testing.allocator;
