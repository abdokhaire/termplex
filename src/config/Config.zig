...
// like the `termplex-build-data` binary depend on the Config but don't
...
pub const compatibility = std.StaticStringMap(
    ...
    // Termplex 1.1 introduced background-blur support for Linux which
    ...
    // Termplex 1.2 renamed all our adw options to gtk because we now have
    // a hard dependency on libadwaita.
    .{ "adw-toolbar-style", cli.compatibilityRenamed(Config, "gtk-toolbar-style") },

    // Termplex 1.2 removed the `hidden` value from `gtk-tabs-location` and
    // moved it to `window-show-tab-bar`.
    .{ "gtk-tabs-location", compatGtkTabsLocation },

    // Termplex 1.2 lets you set `cell-foreground` and `cell-background`
    ...
    // Termplex 1.2 merged `bold-is-bright` into the new `bold-color`
    // by setting the value to "bright".
    .{ "bold-is-bright", compatBoldIsBright },

    // Termplex 1.2 removed the "desktop" option and renamed it to "detect".
    // The semantics also changed slightly but this is the correct mapping.
    .{ "gtk-single-instance", compatGtkSingleInstance },

    // Termplex 1.3 rename the "window" option to "new-window".
    // See: https://github.com/termplex-org/termplex/pull/9764
    .{ "macos-dock-drop-behavior", compatMacOSDockDropBehavior },
});

/// Set Termplex's graphical user interface language to a language other than the
/// system default language. For example:
///
///     language = de
///
/// will force the strings in Termplex's graphical user interface to be in German
/// rather than the system default.
///
/// This will not affect the language used by programs run _within_ Termplex.
/// Those will continue to use the default system language. There are also many
/// non-GUI elements in Termplex that are not translated - this setting will have
/// no effect on those.
///
/// Warning: This setting cannot be reloaded at runtime. To change the language
/// you must fully restart Termplex.
///
/// GTK only.
...
/// etc.
///
/// Notes on emoji specifically: On macOS, Termplex by default will always use
...
/// found, Termplex will use the regular style. This prevents falling back to a
...
/// set `font-family-bold = FooBar` and "FooBar" cannot be found, Termplex will
/// use whatever font is set for `font-family` for the bold style.
///
/// Finally, some styles may be synthesized if they are not supported.
/// For example, if a font does not have an italic style and no alternative
/// italic font is specified, Termplex will synthesize an italic style by
...
/// Control whether Termplex should synthesize a style if the requested style is
/// not available in the specified font-family.
///
/// Termplex can synthesize bold, italic, and bold italic styles if the font
...
///
/// The font feature will apply to all fonts rendered by Termplex. A future
...
///
/// Termplex breaks runs at certain points to improve readability or usability.
/// For example, Termplex by default will break runs under the cursor so that
...
///
/// This configuration only applies to Termplex builds that use FreeType.
...
///     This is the most common setting in GTK apps and therefore also Termplex's
...
/// name, or an absolute path to a custom theme file. Termplex also supports
/// specifying a different theme to use for light and dark mode. Each
/// option is documented below.
///
/// If the theme is an absolute pathname, Termplex will attempt to load that
...
///
/// The first directory is the `themes` subdirectory of your Termplex
/// configuration directory. This is `$XDG_CONFIG_HOME/termplex/themes` or
/// `~/.config/termplex/themes`.
///
/// The second directory is the `themes` subdirectory of the Termplex resources
/// directory. Termplex ships with a multitude of themes that will be installed
/// into this directory. On macOS, this list is in the
/// `Termplex.app/Contents/Resources/termplex/themes` directory. On Linux, this
/// list is in the `share/termplex/themes` directory (wherever you installed the
/// Termplex "share" directory.
///
/// To see a list of available themes, run `termplex +list-themes`.
///
/// A theme file is simply another Termplex configuration file. They share
...
/// `config-file`. At the time of writing this, Termplex will not show any
...
background: Color = .{ .r = 0x0f, .g = 0x19, .b = 0x23 },

/// Foreground color for the window.
/// Specified as either hex (`#RRGGBB` or `RRGGBB`) or a named X11 color.
foreground: Color = .{ .r = 0xe2, .g = 0xe8, .b = 0xf0 },
...
/// splits. A future improvement to Termplex will address this.
///
...
/// memory usage (specifically VRAM usage). A future Termplex improvement
...
///
/// If this is not set at all (`null`), then Termplex will respect DEC Mode 12
...
/// Termplex's shell integration features to be enabled.
///
...
///   anything but modifiers or keybinds that are processed by Termplex).
///
...
/// to all scrolling devices. Specifying a prefix was introduced in Termplex
/// 1.2.1.
///
...
///
/// On macOS, changing this configuration requires restarting Termplex completely.
...
/// Control when Termplex preserves a zoomed split. Under normal circumstances,
...
/// created when Termplex starts, use the `initial-command` configuration.
///
/// Termplex supports the common `-e` flag for executing a command with
/// arguments. For example, `termplex -e fish --with --custom --args`.
/// This flag sets the `initial-command` configuration, see that for more
/// information.
command: ?Command = null,

/// This is the same as "command", but only applies to the first terminal
/// surface created when Termplex starts. Subsequent terminal surfaces will use
...
///
/// If you're using the `termplex` CLI there is also a shortcut to set this
/// with arguments directly: you can use the `-e` flag. For example: `termplex -e
...
///     the `termplex -e` command).
///
///   * `gtk-single-instance=false` - This ensures that a new instance is
///     launched and the CLI args are respected.
///
///   * `quit-after-last-window-closed=true` - This ensures that the Termplex
...
/// variables set by Termplex. For example, if you set `TERMPLEX_RESOURCES_DIR`
/// then the value you set here will override the value Termplex typically
/// automatically injects.
///
/// These environment variables _will not_ be passed to commands run by Termplex
...
/// The title Termplex will use for the window. This will force the title of the
/// window to be this title at all times and Termplex will ignore any set title
...
/// bus name that Termplex uses to connect to DBus.
///
/// Note that changing this value between invocations will create new, separate
/// instances, of Termplex when running with `gtk-single-instance=true`. See that
/// option for more details.
///
/// Changing this value may break launching Termplex from `.desktop` files, via
/// DBus activation, or systemd user services as the system is expecting Termplex
/// to connect to DBus using the default `class` when it is launched.
///
/// The class name must follow the requirements defined [in the GTK
/// documentation](https://docs.gtk.org/gio/type_func.Application.id_is_valid.html).
///
/// The default is `com.termplex.app`.
///
...
/// setting. If a previous Termplex terminal exists in the same process,
...
/// if Termplex can detect it is launched from launchd (double-clicked) or
/// `open`, then it defaults to `home`. On Linux with GTK, if Termplex can detect
...
/// the documentation or using the `termplex +list-actions` command.
///
...
/// otherwise bound. When looking up a binding, Termplex first tries to match
...
///
/// Warning: If you define a sequence as a CLI argument to `termplex`,
/// you probably have to quote the keybind since `>` is a special character
/// in most shells. Example: termplex --keybind='ctrl+a>n=new_window'
///
/// A trigger sequence has some special handling:
///
///   * Termplex will wait an indefinite amount of time for the next key in
...
///  * `global:`
///
///    Make the keybind global. By default, keybinds only work within Termplex
...
///    (e.g. even when Termplex is not focused), specify this prefix.
...
///    if there is a selection to copy. If there is no selection, Termplex
...
/// to Termplex. When a `global:` keybind is specified and Termplex is launched
/// or reloaded, Termplex will attempt to request these permissions.
...
/// sequence.
///
/// Chained keybinds are available since Termplex 1.3.0.
///
/// ## Key Tables
///
...
///
/// Key tables are available since Termplex 1.3.0.
keybind: Keybinds = .{},

/// Remap modifier keys within Termplex. This allows you to swap or reassign
...
///
/// This will NOT change keyboard layout or key encodings outside of Termplex.
...
/// before Termplex receives the key events.
///
/// Example:
///
...
///   prior to Termplex receiving them. For example, `option+a` on macOS
...
///   this happens before Termplex receives the key event. To workaround
...
/// may override this preference. Termplex will do its best to respect this
...
///    desktop environment. This option usually makes Termplex look the most
...
///   * `termplex` - Use the background and foreground colors specified in the
///     Termplex configuration. This is only supported on Linux builds.
///
/// On macOS, if `macos-titlebar-style` is `tabs` or `transparent`, the window theme will be
/// automatically set based on the luminosity of the terminal background color.
/// This only applies to terminal windows. This setting will still apply to
/// non-terminal windows within Termplex.
///
...
/// certain size to fit within the grid. There is nothing Termplex will do about
...
/// to be a certain position to fit within the grid. There is nothing Termplex
...
///
///   * `always` will always save window state whenever Termplex is exited.
///
/// If you change this value to `never` while Termplex is not running, the next
/// Termplex launch will NOT restore the window state.
///
/// If you change this value to `default` while Termplex is not running and the
/// previous exit saved state, the next Termplex launch will still restore the
/// window state. This is because Termplex cannot know if the previous exit was
/// due to a forced save or not (macOS doesn't provide this information).
///
/// If you change this value so that window state is saved while Termplex is not
/// running, the previous window state will not be restored because Termplex only
...
/// window-theme is set to termplex. Currently only supported in the GTK app
/// runtime.
///
/// Specified as either hex (`#RRGGBB` or `RRGGBB`) or a named X11 color.
...
/// config path ($XDG_CONFIG_HOME/termplex/config.termplex).
///
/// If this is false, the default configuration paths will not be loaded.
/// This is targeted directly at using Termplex from the CLI in a way
...
///
/// On Linux, if this is `true`, Termplex can delay quitting fully until a
/// configurable amount of time has passed after the last window is closed.
/// See the documentation of `quit-after-last-window-closed-delay`.
@"quit-after-last-window-closed": bool = builtin.os.tag == .linux,

/// Controls how long Termplex will stay running after the last open surface has
...
/// Termplex will quit immediately after the last window is closed if
/// `quit-after-last-window-closed` is `true`.
///
/// Only implemented on Linux.
@"quit-after-last-window-closed-delay": ?Duration = null,

/// This controls whether an initial window is created when Termplex
/// is run. Note that if `quit-after-last-window-closed` is `true` and
/// `quit-after-last-window-closed-delay` is set, setting `initial-window` to
/// `false` will mean that Termplex will quit after the configured delay if no
...
///
/// On macOS, changing this configuration requires restarting Termplex
/// completely.
///
...
///
/// Available since: 1.2.0
@"gtk-quick-terminal-namespace": [:0]const u8 = "termplex-quick-terminal",
...
///     converts TERM from `xterm-termplex` to `xterm-256color` when connecting to
...
///     Attempts to install Termplex's terminfo entry using `infocmp` and `tic` when
...
///     (Available since: 1.2.0)
///
///   * `path` - Add Termplex's binary directory to PATH. This ensures the `termplex`
...
/// when both `ssh-env` and `ssh-terminfo` are enabled, Termplex will install its
/// terminfo on remote hosts and use `xterm-termplex` as TERM, falling back to
...
/// command-palette-entry = title:"Termplex",description:"Add a little Termplex to your terminal.",action:"text:\xf0\x9f\x91\xbb"
/// ```
///
...
/// Termplex currently supports OSC 10 (foreground), OSC 11 (background), and
...
///
/// Warning: Invalid shaders can cause Termplex to become unusable such as by
...
/// largely match Shadertoy, with some caveats and Termplex-specific extensions.
///
...
///
///    Request the user's attention when Termplex is unfocused, until it has
...
///      bring the Termplex window into focus;
///
...
/// Control the in-app notifications that Termplex shows.
///
...
/// and not something Termplex enables.
///
...
/// onto the Termplex icon in the macOS dock.
///
/// Valid values are:
///
...
/// If true, Termplex on macOS will automatically enable the "Secure Input"
/// feature when it detects that a password prompt is being displayed.
///
/// "Secure Input" is a macOS security feature that prevents applications from
/// reading keyboard events. This can always be enabled manually using the
/// `Termplex > Secure Keyboard Entry` menu item.
///
...
/// If true, Termplex will show a graphical indication when secure input is
...
/// If true, Termplex exposes and handles the built-in AppleScript dictionary
/// on macOS.
///
...
/// Valid values:
///
///  * `official` - Use the official Termplex icon.
///  * `blueprint`, `chalkboard`, `microchip`, `glass`, `holographic`,
///    `paper`, `retro`, `xray` - Official variants of the Termplex icon
...
///  * `custom-style` - Use the official Termplex icon but with custom
...
///
/// Defaults to `~/.config/termplex/Termplex.icns`
...
/// Whether macOS Shortcuts are allowed to control Termplex.
///
/// Termplex exposes a number of actions that allow Shortcuts to
/// control and interact with Termplex. This includes creating new
...
/// Valid values are:
///
/// * `ask` - Ask the user whether for permission. Termplex will remember
///   this choice and never ask again. This is similar to other macOS
///   permissions such as microphone access, camera access, etc.
///
/// * `allow` - Allow Shortcuts to control Termplex without asking.
///
/// * `deny` - Deny Shortcuts from controlling Termplex.
///
/// Available since: 1.2.0
...
/// instead of the entire Termplex process. Similarly, if a shell program is
/// using too much CPU, only that surface will be CPU-throttled.
///
/// This will cause startup times to be slower (a hundred milliseconds or so),
/// so the default value is "single-instance." In single-instance mode, only
/// one instance of Termplex is running (see gtk-single-instance) so the startup
/// time is a one-time cost. Additionally, single instance Termplex is much
/// more likely to have many windows, tabs, etc. so cgroup isolation is a
/// big benefit.
///
/// This feature requires `systemd`. If `systemd` is unavailable, cgroup
/// initialization will fail. By default, this will not prevent Termplex from
...
///   * `single-instance` - Enable cgroups only for Termplex instances launched
...
/// If `true`, the Termplex GTK application will run in single-instance mode:
/// each new `termplex` process launched will result in a new window if there is
/// already a running process.
///
/// If `false`, each new termplex process will launch a separate application.
///
/// If `detect`, Termplex will assume true (single instance) unless one of
/// the following scenarios is found:
///
/// 1. TERM_PROGRAM environment variable is a non-empty value. In this
/// case, we assume Termplex is being launched from a graphical terminal
/// session and you want a dedicated instance.
///
/// 2. Any CLI arguments exist. In this case, we assume you are passing
/// custom Termplex configuration. Single instance mode inherits the
...
/// The default value is `detect`.
///
/// Note that debug builds of Termplex have a separate single-instance ID
...
/// If `true` (default), then the Termplex GTK tabs will be "wide." Wide tabs
...
///
/// Launch Termplex with `env GTK_DEBUG=interactive termplex` to tweak Termplex's
/// CSS in real time using the GTK Inspector. Errors in your CSS files would
/// also be reported in the terminal you started Termplex from. See
...
/// escape sequences such as OSC 4. (Since Termplex 1.2.0, the previous
/// configuration `bold-is-bright` is deprecated and replaced by this
/// usage).
///
/// Available since Termplex 1.2.0.
...
/// keyprotocol=termplex:kitty`, however a bug in the implementation prevents it
/// from working properly. https://github.com/vim/vim/pull/13211 fixes this.
term: []const u8 = "xterm-termplex",
...
///
/// If the selected backend is not available on the platform, Termplex will
...
/// Control the auto-update functionality of Termplex. This is only supported
/// on macOS currently, since Linux builds are distributed via package
/// managers that are not centrally controlled by Termplex.
///
...
/// running Termplex version. If you download a pre-release version of Termplex
/// then this will be set to `tip` and you will receive pre-release updates.
/// If you download a stable version of Termplex then this will be set to
...
/// Set to true if Termplex was executed as xdg-terminal-exec on Linux.
...
test "handle bom in config files" {
...

    {
        ...
        try cfg.loadReader(
            alloc,
            &reader,
            "/home/termplex/.config/termplex/config.termplex",
        );
        try cfg.finalize();
...
pub fn finalize(self: *Config) !void {
    ...
    if (self.theme) |theme| {
        ...
        if (different) {
            ...
            // because it'll force the theme based on the Termplex theme.
    ...
    // short value because it can cause Termplex to quit before the first
...
/// Termplex in a CLI environment. We need this to change some behaviors.
...
/// If this is changed, you must also update termplex.h
...
pub const WorkingDirectory = union(enum) {
    ...
    test "WorkingDirectory parseCLI" {
        ...
        try wd.parseCLI(alloc, "~/projects/termplex");
        try testing.expectEqualStrings("~/projects/termplex", wd.path);
    ...
    test "WorkingDirectory finalize" {
        ...
        {
            var wd: Self = .{ .path = "~/projects/termplex" };
            try wd.finalize(alloc);
...
pub const Keybinds = struct {
    ...
    pub fn init(self: *Keybinds, alloc: Allocator) !void {
        ...
        }
        {
            ...
            inline while (i <= end) : (i += 1) {
                ...
                // libtermplex trigger API returns this one for the action,
    ...
    // Regression test for https://github.com/termplex-org/termplex/issues/2734
    test "formatConfig multiple items" {
        const testing = std.testing;
        var buf: std.Io.Writer.Allocating = .init(testing.allocator);
        defer buf.deinit();
...
/// See macos-icon
///
/// Note: future versions of Termplex can support a custom icon with
...
pub const FreetypeLoadFlags = packed struct {
    ...
    // for Freetype itself. Termplex hasn't made any opinionated changes
    // to these defaults. (Strictly speaking, `light` isn't FreeType's
    // own default, but appears to be the effective default with most
    // Fontconfig-aware software using FreeType, so until Termplex
...
pub const WindowDecoration = enum(c_int) {
    ...
    pub const getGObjectType = switch (build_config.app_runtime) {
        .gtk => @import("gobject").ext.defineEnum(
            WindowDecoration,
            .{ .name = "TermplexConfigWindowDecoration" },
        ),
...
test "working-directory expands tilde" {
    ...
    defer cfg.deinit();
    var it: TestIterator = .{ .data = &.{
        "--working-directory=~/projects/termplex",
    } };
    try cfg.loadIter(alloc, &it);
    try cfg.finalize();
