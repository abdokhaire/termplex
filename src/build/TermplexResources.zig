...
pub fn init(b: *std.Build, cfg: *const Config, deps: *const SharedDeps) !TermplexResources {
    ...
    terminfo: {
        ...
        const source = wf.addCopyFile(run.captureStdOut(), "termplex.terminfo");

        if (cfg.emit_terminfo) {
            const source_install = b.addInstallFile(
                source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/termplex.terminfo"
                else
                    "share/terminfo/termplex.terminfo",
            );
        ...
        if (cfg.emit_termcap) {
            ...
            const cap_install = b.addInstallFile(
                out_source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/termplex.termcap"
                else
                    "share/terminfo/termplex.termcap",
            );
    ...
    // Shell-integration
    {
        const install_step = b.addInstallDirectory(.{
            ...
            .install_subdir = b.pathJoin(&.{ "termplex", "shell-integration" }),
    ...
    if (cfg.emit_themes) {
        if (b.lazyDependency("iterm2_themes", .{})) |upstream| {
            const install_step = b.addInstallDirectory(.{
                ...
                .install_dir = .{ .custom = "share" },
                .install_subdir = b.pathJoin(&.{ "termplex", "themes" }),
                .exclude_extensions = &.{".md"},
            });
            try steps.append(b.allocator, &install_step.step);
        }
    ...
    // Fish shell completions
    {
        ...
        run.addArg("+fish");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "termplex.fish");
    ...
    // zsh shell completions
    {
        ...
        run.addArg("+zsh");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "_termplex");
    ...
    // bash shell completions
    {
        ...
        run.addArg("+bash");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "termplex.bash");
    ...
    // Vim and Neovim plugin
    {
...

        {
            ...
            _ = wf.addCopyFile(run.captureStdOut(), "syntax/termplex.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-ftdetect");
            _ = wf.addCopyFile(run.captureStdOut(), "ftdetect/termplex.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-ftplugin");
            _ = wf.addCopyFile(run.captureStdOut(), "ftplugin/termplex.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-compiler");
            _ = wf.addCopyFile(run.captureStdOut(), "compiler/termplex.vim");
        }
    ...
    // 'termplex.sublime-syntax' file from zig-out to the '~.config/bat/syntaxes'
    // directory. The syntax then needs to be mapped to the correct language in
    // the config file within the '~.config/bat' directory
    // (ex: --map-syntax "/Users/user/.config/termplex/config.termplex:Termplex Config").
    {
        const run = b.addRunArtifact(build_data_exe);
        run.addArg("+sublime");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "termplex.sublime-syntax");
...
/// Add the resource files needed to make Termplex a proper
...
fn addLinuxAppResources(
    ...
    const app_id = b.fmt("com.termplex.app{s}", .{
        switch (cfg.optimize) {
            .Debug, .ReleaseSafe => "-debug",
            .ReleaseFast, .ReleaseSmall => "",
        },
    ...
    const templates: []const Template = templates: {
        ...
        // within app stores
        try ts.append(b.allocator, .{
            b.path("dist/linux/com.termplex.app.metainfo.xml.in"),
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/termplex_dolphin.desktop"),
        "share/kio/servicemenus/com.termplex.app.desktop",
    ).step);

    // Right click menu action for Nautilus. Note that this _must_ be named
    // `termplex.py`. Using the full app id causes problems (see #5468).
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/termplex_nautilus.py"),
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        ...
        "share/icons/hicolor/16x16/apps/com.termplex.app.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/32.png"),
        "share/icons/hicolor/32x32/apps/com.termplex.app.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/128.png"),
        "share/icons/hicolor/128x128/apps/com.termplex.app.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/256.png"),
        "share/icons/hicolor/256x256/apps/com.termplex.app.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/512.png"),
        "share/icons/hicolor/512x512/apps/com.termplex.app.png",
    ).step);
    // Flatpaks only support icons up to 512x512.
    if (!cfg.flatpak) {
        try steps.append(b.allocator, &b.addInstallFile(
            b.path("images/gnome/1024.png"),
            "share/icons/hicolor/1024x1024/apps/com.termplex.app.png",
        ).step);
    }

    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/32.png"),
        "share/icons/hicolor/16x16@2/apps/com.termplex.app.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/64.png"),
        "share/icons/hicolor/32x32@2/apps/com.termplex.app.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/256.png"),
        "share/icons/hicolor/128x128@2/apps/com.termplex.app.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/512.png"),
        "share/icons/hicolor/256x256@2/apps/com.termplex.app.png",
    ).step);
}

pub fn install(self: *const TermplexResources) void {
    const b = self.steps[0].owner;
    self.addStepDependencies(b.getInstallStep());
}

pub fn addStepDependencies(
    self: *const TermplexResources,
