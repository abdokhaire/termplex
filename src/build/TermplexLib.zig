...
pub fn initShared(
    ...
    const dsymutil: ?std.Build.LazyPath = dsymutil: {
        ...
        dsymutil.addArgs(&.{"-o"});
        const output = dsymutil.addOutputFileArg("libtermplex.dSYM");
...
pub fn install(self: *const TermplexLib, name: []const u8) void {
    const b = self.step.owner;
    const lib_install = b.addInstallLibFile(self.output, name);
    b.getInstallStep().dependOn(&lib_install.step);
}

pub fn installHeader(self: *const TermplexLib) void {
    const b = self.step.owner;
    const header_install = b.addInstallHeaderFile(
        b.path("include/termplex.h"),
        "termplex.h",
    );
