...
pub fn init(b: *std.Build, cfg: *const Config) !TermplexDist {
    ...
    {
        const framedata = TermplexFrameData.distResources(b);
    ...
    // embed the Termplex version in the tarball
    {
        ...
        git_archive.addArg(b.fmt("--prefix=termplex-{f}/", .{
            cfg.version,
        }));
    ...
    for (resources.items) |resource| {
        ...
        // --add-file uses the most recent --prefix to determine the path
        // in the archive to copy the file (the directory only).
        git_archive.addArg(b.fmt("--prefix=termplex-{f}/{s}/", .{
            cfg.version,
    ...
    git_archive.addArgs(&.{
        ...
        b.fmt("--prefix=termplex-{f}/", .{cfg.version}),
        "-o",
    });
    ...
    const install = b.addInstallFile(
        output,
        b.fmt("dist/termplex-{f}.tar.gz", .{cfg.version}),
    );
    ...
    // This is the root Termplex source dir of the extracted source tarball.
    // i.e. this is way `build.zig` is.
    const extract_dir = check
        .addOutputDirectoryArg("termplex")
        .path(b, b.fmt("termplex-{f}", .{cfg.version}));
...
/// of the source tarball with Termplex. These aren't committed to the Git
