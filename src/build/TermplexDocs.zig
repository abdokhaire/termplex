//! TermplexDocs generates all the on-disk documentation that Termplex is
//! installed with (man pages, html, markdown, etc.)
const TermplexDocs = @This();
...
pub fn init(
    ...
    const manpages = [_]struct {
        ...
        .{ .name = "termplex", .section = "1" },
        .{ .name = "termplex", .section = "5" },
    };
    ...
    inline for (manpages) |manpage| {
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            markdown_output,
            "share/termplex/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".md",
        ).step);
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            ...
            "share/termplex/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".html",
        ).step);
...
/// app.
pub fn installDummy(self: *const TermplexDocs, step: *std.Build.Step) void {
    _ = self;
