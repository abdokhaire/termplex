...
pub fn init(b: *std.Build, cfg: *const Config) !TermplexI18n {
    _ = cfg;
...
pub fn install(self: *const TermplexI18n) void {
    self.addStepDependencies(self.owner.getInstallStep());
}

pub fn addStepDependencies(
    self: *const TermplexI18n,
...
fn createUpdateStep(b: *std.Build) !*std.Build.Step {
    ...
    inline for (gresource.blueprints) |blp| {
        ...
        // a lot of churn because not everyone has the Termplex code checked out in
...

    {
        ...
        for (gtk_files.items) |item| {
            ...
            // cause a lot of churn because not everyone has the Termplex code
    ...
    const nautilus_script_path = "dist/linux/termplex_nautilus.py";
    ...
    const xgettext_merge = b.addSystemCommand(&.{
        ...
        "--copyright-holder=\"Mitchell Hashimoto, Termplex contributors\"",
        "-o",
        "-",
    });
