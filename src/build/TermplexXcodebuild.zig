...
const Docs = @import("TermplexDocs.zig");
const I18n = @import("TermplexI18n.zig");
const Resources = @import("TermplexResources.zig");
const XCFramework = @import("TermplexXCFramework.zig");
...
pub fn init(
    ...
    const app_path = b.fmt("macos/build/{s}/Termplex.app", .{xc_config});

    // Our step to build the Termplex macOS app.
    const build = build: {
    ...
    // Our step to open the resulting Termplex app.
    const open = open: {
        ...
        const open = RunStep.create(b, "run Termplex app");
        open.has_side_effects = true;
        open.cwd = b.path("");
        open.addArgs(&.{b.fmt(
            "{s}/Contents/MacOS/termplex",
            .{app_path},
        )});
        ...
        open.setEnvironmentVariable("TERMPLEX_LOG", "stderr,macos");

        // Configure how we're launching
        open.setEnvironmentVariable("TERMPLEX_MAC_LAUNCH_SOURCE", "zig_run");
...
pub fn install(self: *const Termplex) void {
    const b = self.copy.step.owner;
    b.getInstallStep().dependOn(&self.copy.step);
}

pub fn installXcframework(self: *const Termplex) void {
