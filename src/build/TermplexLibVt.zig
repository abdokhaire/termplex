const TermplexLibVt = @This();

const std = @import("std");
const builtin = @import("builtin");
const assert = std.debug.assert;
const RunStep = std.Build.Step.Run;
const TermplexZig = @import("TermplexZig.zig");
...
pub fn initShared(
    ...
    const dsymutil: ?std.Build.LazyPath = dsymutil: {
        ...
        dsymutil.addArgs(&.{"-o"});
        const output = dsymutil.addOutputFileArg("libtermplex-vt.dSYM");
    ...
    const pc: std.Build.LazyPath = pc: {
        ...
        break :pc wf.add("libtermplex-vt.pc", b.fmt(
            \\prefix={s}
            \\includedir=${{prefix}}/include
            \\libdir=${{prefix}}/lib
            \\
            \\Name: libtermplex-vt
            \\URL: https://github.com/termplex-org/termplex
            \\Description: Termplex VT library
            \\Version: 0.1.0
            \\Cflags: -I${{includedir}}
            \\Libs: -L${{libdir}} -ltermplex-vt
        , .{b.install_prefix}));
    };
...
pub fn install(
    ...
    if (self.pkg_config) |pkg_config| {
        step.dependOn(&b.addInstallFileWithDir(
            pkg_config,
            .prefix,
            "share/pkgconfig/libtermplex-vt.pc",
        ).step);
    }
}
