...
pub const Target = union(Key) {
    ...
    // Sync with: termplex_target_tag_e
    pub const Key = enum(c_int) {
        app,
        surface,

        test "termplex.h Target.Key" {
            try lib.checkTermplexHEnum(Key, "TERMPLEX_TARGET_");
        }
    };

    // Sync with: termplex_target_u
    pub const CValue = extern union {
        app: void,
        surface: *apprt.Surface,
    };

    // Sync with: termplex_target_s
    pub const C = extern struct {
        key: Key,
        value: CValue,
    };

    /// Convert to termplex_target_s.
...
pub const Action = union(Key) {
    ...
    //    because it maps directly to the libtermplex C enum. For ABI
    ...
    //
    // 4. Update `include/termplex.h`: add the new key, value, and union
    ...
    /// Toggle the visibility of all Termplex terminal windows.
    toggle_visibility,
    ...
    /// Open the Termplex configuration. This is platform-specific about
    ...
    ///
    /// Despite the name, this is the notification that libtermplex sends
    ...
    /// Sync with: termplex_action_tag_e
    pub const Key = enum(c_int) {
        quit,
        new_window,
        new_tab,
        ...
        test "termplex.h Action.Key" {
            try lib.checkTermplexHEnum(Key, "TERMPLEX_ACTION_");
        }
    };

    /// Sync with: termplex_action_u
...
pub const SplitDirection = enum(c_int) {
    ...
    test "termplex.h SplitDirection" {
        try lib.checkTermplexHEnum(SplitDirection, "TERMPLEX_SPLIT_DIRECTION_");
    }
};
...
pub const GotoSplit = enum(c_int) {
    ...
    test "termplex.h GotoSplit" {
        try lib.checkTermplexHEnum(GotoSplit, "TERMPLEX_GOTO_SPLIT_");
    }
};
...
pub const GotoWindow = enum(c_int) {
    ...
    test "termplex.h GotoWindow" {
        try lib.checkTermplexHEnum(GotoWindow, "TERMPLEX_GOTO_WINDOW_");
    }
};
...
pub const ResizeSplit = extern struct {
    ...
    pub const Direction = enum(c_int) {
        ...
        test "termplex.h ResizeSplit.Direction" {
            try lib.checkTermplexHEnum(Direction, "TERMPLEX_RESIZE_SPLIT_");
        }
    };
};
...
pub const GotoTab = enum(c_int) {
    ...
    // test "termplex.h GotoTab" {
    //     try lib.checkTermplexHEnum(GotoTab, "TERMPLEX_GOTO_TAB_");
    // }
};
...
pub const Fullscreen = enum(c_int) {
    ...
    test "termplex.h Fullscreen" {
        try lib.checkTermplexHEnum(Fullscreen, "TERMPLEX_FULLSCREEN_");
    }
};
...
pub const FloatWindow = enum(c_int) {
    ...
    test "termplex.h FloatWindow" {
        try lib.checkTermplexHEnum(FloatWindow, "TERMPLEX_FLOAT_WINDOW_");
    }
};
...
pub const SecureInput = enum(c_int) {
    ...
    test "termplex.h SecureInput" {
        try lib.checkTermplexHEnum(SecureInput, "TERMPLEX_SECURE_INPUT_");
    }
};
...
pub const Inspector = enum(c_int) {
    ...
    test "termplex.h Inspector" {
        try lib.checkTermplexHEnum(Inspector, "TERMPLEX_INSPECTOR_");
    }
};
...
pub const QuitTimer = enum(c_int) {
    ...
    test "termplex.h QuitTimer" {
        try lib.checkTermplexHEnum(QuitTimer, "TERMPLEX_QUIT_TIMER_");
    }
};
...
pub const Readonly = enum(c_int) {
    ...
    test "termplex.h Readonly" {
        try lib.checkTermplexHEnum(Readonly, "TERMPLEX_READONLY_");
    }
};
...
pub const MouseVisibility = enum(c_int) {
    ...
    test "termplex.h MouseVisibility" {
        try lib.checkTermplexHEnum(MouseVisibility, "TERMPLEX_MOUSE_");
    }
};
...
pub const PromptTitle = enum(c_int) {
    ...
    test "termplex.h PromptTitle" {
        try lib.checkTermplexHEnum(PromptTitle, "TERMPLEX_PROMPT_TITLE_");
    }
};
...
pub const MouseOverLink = struct {
    ...
    // Sync with: termplex_action_mouse_over_link_s
    pub const C = extern struct {
        url: [*]const u8,
        len: usize,
    };
...
pub const SetTitle = struct {
    ...
    // Sync with: termplex_action_set_title_s
    pub const C = extern struct {
        title: [*:0]const u8,
    };
...
pub const Pwd = struct {
    ...
    // Sync with: termplex_action_set_pwd_s
    pub const C = extern struct {
        pwd: [*:0]const u8,
    };
...
pub const DesktopNotification = struct {
    ...
    // Sync with: termplex_action_desktop_notification_s
    pub const C = extern struct {
        title: [*:0]const u8,
        body: [*:0]const u8,
    };
...
pub const KeySequence = union(enum) {
    ...
    // Sync with: termplex_action_key_sequence_s
    pub const C = extern struct {
        active: bool,
...
pub const KeyTable = union(enum) {
    ...
    // Sync with: termplex_action_key_table_tag_e
    ...
    // Sync with: termplex_action_key_table_u
    pub const CValue = extern union {
    ...
    // Sync with: termplex_action_key_table_s
    pub const C = extern struct {
        tag: Tag,
        value: CValue,
    };
...
pub const ColorKind = enum(c_int) {
    ...
    // test "termplex.h ColorKind" {
    //     try lib.checkTermplexHEnum(ColorKind, "TERMPLEX_COLOR_KIND_");
    // }
};

pub const ReloadConfig = extern struct {
    /// A soft reload means that the configuration doesn't need to be
    /// read off disk, but libtermplex needs the full config again so call
...
pub const ConfigChange = struct {
    ...
    // Sync with: termplex_action_config_change_s
    pub const C = extern struct {
        config: *const configpkg.Config,
    };
...
pub const OpenUrl = struct {
    ...
    ///
    /// Sync with: termplex_action_open_url_kind_e
    pub const Kind = enum(c_int) {
        ...
        test "termplex.h OpenUrl.Kind" {
            try lib.checkTermplexHEnum(Kind, "TERMPLEX_ACTION_OPEN_URL_KIND_");
        }
    };

    // Sync with: termplex_action_open_url_s
    pub const C = extern struct {
        kind: Kind,
        url: [*]const u8,
        len: usize,
...
/// sync with termplex_action_close_tab_mode_e in termplex.h
pub const CloseTabMode = enum(c_int) {
    ...
    test "termplex.h CloseTabMode" {
        try lib.checkTermplexHEnum(CloseTabMode, "TERMPLEX_ACTION_CLOSE_TAB_MODE_");
    }
};
...
pub const CommandFinished = struct {
    ...
    /// sync with termplex_action_command_finished_s in termplex.h
    pub const C = extern struct {
        exit_code: i16,
        duration: u64,
    };
...
pub const StartSearch = struct {
    ...
    // Sync with: termplex_action_start_search_s
    pub const C = extern struct {
        needle: [*:0]const u8,
    };
...
pub const SearchTotal = struct {
    ...
    // Sync with: termplex_action_search_total_s
    pub const C = extern struct {
        total: isize,
    };
...
pub const SearchSelected = struct {
    ...
    // Sync with: termplex_action_search_selected_s
    pub const C = extern struct {
        selected: isize,
    };
