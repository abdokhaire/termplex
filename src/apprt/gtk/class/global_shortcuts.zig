...
const log = std.log.scoped(.gtk_termplex_global_shortcuts);
...
pub const GlobalShortcuts = extern struct {
    ...
    /// Get the XDG portal request path for the current Termplex instance.
    ///
...
fn generateToken(buf: *Token) [:0]const u8 {
    // u28 takes up 7 bytes in hex, 8 bytes for "termplex_" and 1 byte for NUL
    // 7 + 8 + 1 = 16
