...
/// The bus name of the Termplex instance that we are calling.
bus_name: [:0]const u8,

/// The object path of the Termplex instance that we are calling.
...
pub fn init(alloc: Allocator, target: apprt.ipc.Target, action: [:0]const u8) (Allocator.Error || std.Io.Writer.Error || apprt.ipc.Errors)!Self {
    ...
    // Termplex instance we're interested in.
...
/// Send the IPC to the remote Termplex. Once it completes, nothing further
