...
//
// `termplex +new-window` is equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest com.termplex.app --object-path /com/mitchellh/termplex --method org.gtk.Actions.Activate new-window [] []
// ```
//
// `termplex +new-window -e echo hello` would be equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest com.termplex.app --object-path /com/mitchellh/termplex --method org.gtk.Actions.Activate new-window-command '[<@as ["-e" "echo" "hello"]>]' []
// ```
pub fn newWindow(alloc: Allocator, target: apprt.ipc.Target, value: apprt.ipc.Action.NewWindow) (Allocator.Error || std.Io.Writer.Error || apprt.ipc.Errors)!bool {
    ...
    if (value.arguments) |arguments| {
        ...
        // will be sent to the main Termplex instance and interpreted as CLI
        // arguments.
