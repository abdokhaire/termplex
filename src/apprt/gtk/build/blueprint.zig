...
pub const blueprint_compiler_help =
    \\
    \\When building from a Git checkout, Termplex requires
    \\version {f} or newer of `blueprint-compiler` as a
    \\build-time dependency. Please install it, ensure that it
    \\is available on your PATH, and then retry building Termplex.
    \\See `HACKING.md` for more details.
    \\
    \\This message should *not* appear for normal users, who
    \\should build Termplex from official release tarballs instead.
    \\Please consult https://termplex.org/docs/install/build for
...
pub fn main() !void {
    ...
    if (adwaita_version.order(required_adwaita_version) == .lt) {
        std.debug.print(
            ...
            \\
            \\Termplex requires a version {f} or newer of `libadwaita` to
            \\compile this blueprint. Please install it, ensure that it is
            \\available on your PATH, and then retry building Termplex.
