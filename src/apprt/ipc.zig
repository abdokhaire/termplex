//! Inter-process Communication to a running Termplex instance from a separate
//! process.
...
pub const Target = union(Key) {
    /// Open up a new window in a custom instance of Termplex.
    class: [:0]const u8,

    /// Detect which instance to open a new window in.
    detect,

    // Sync with: termplex_ipc_target_tag_e
    pub const Key = enum(c_int) {
        class,
        detect,

        test "termplex.h Target.Key" {
            try lib.checkTermplexHEnum(Key, "TERMPLEX_IPC_TARGET_");
        }
    };

    // Sync with: termplex_ipc_target_u
    pub const CValue = extern union {
        class: [*:0]const u8,
        detect: void,
    };

    // Sync with: termplex_ipc_target_s
    pub const C = extern struct {
        key: Key,
        value: CValue,
    };

    /// Convert to termplex_ipc_target_s.
...
pub const Action = union(enum) {
    ...
    /// Sync with: termplex_ipc_action_u
    ...
    /// Sync with: termplex_ipc_action_s
    pub const C = extern struct {
        key: Key,
        value: CValue,
    };
    ...
    /// Convert to termplex_ipc_action_s.
