...
pub const Option = enum {
    ...
    // by Termplex with the "last" option. See Redraw the type for more details.
    redraw,
    ...
    // Termplex will then send a click event to the shell when the user
    ...
    // Termplex may generate a number of fake key events to move the cursor
...
/// started by Kitty[1] and extended by Termplex (the "last" option).
///
...
pub const Redraw = enum(u2) {
    ...
    /// The shell does NOT support redrawing. In this case, Termplex will NOT
    /// clear any prompt lines on resize.
    false,

    /// The shell supports redrawing only the LAST line of the prompt.
    /// Termplex will only clear the last line of the prompt on resize.
    ///
