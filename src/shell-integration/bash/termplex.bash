...
if [ -n "$TERMPLEX_BASH_INJECT" ]; then
  ...
  builtin declare __termplex_bash_flags="$TERMPLEX_BASH_INJECT"
  builtin unset ENV TERMPLEX_BASH_INJECT

  # Restore an existing ENV that was replaced by the shell integration code.
  if [[ -n "$TERMPLEX_BASH_ENV" ]]; then
    builtin export ENV=$TERMPLEX_BASH_ENV
    builtin unset TERMPLEX_BASH_ENV
  fi
  ...
  if [[ -n "$TERMPLEX_BASH_UNEXPORT_HISTFILE" ]]; then
    builtin export -n HISTFILE
    builtin unset TERMPLEX_BASH_UNEXPORT_HISTFILE
  fi
  ...
  if builtin shopt -q login_shell; then
    if [[ $__termplex_bash_flags != *"--noprofile"* ]]; then
      [ -r /etc/profile ] && builtin source "/etc/profile"
      for __termplex_rcfile in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
        [ -r "$__termplex_rcfile" ] && {
          builtin source "$__termplex_rcfile"
          break
        }
      done
    fi
  else
    if [[ $__termplex_bash_flags != *"--norc"* ]]; then
      ...
      for __termplex_rcfile in /etc/bash.bashrc /etc/bash/bashrc /etc/bashrc; do
        [ -r "$__termplex_rcfile" ] && {
          builtin source "$__termplex_rcfile"
          break
        }
      done
      if [[ -z "$TERMPLEX_BASH_RCFILE" ]]; then TERMPLEX_BASH_RCFILE="$HOME/.bashrc"; fi
      [ -r "$TERMPLEX_BASH_RCFILE" ] && builtin source "$TERMPLEX_BASH_RCFILE"
    fi
  fi

  builtin unset __termplex_rcfile
  builtin unset __termplex_bash_flags
  builtin unset TERMPLEX_BASH_RCFILE
fi

# Add Termplex binary to PATH if the path feature is enabled
if [[ "$TERMPLEX_SHELL_FEATURES" == *"path"* && -n "$TERMPLEX_BIN_DIR" ]]; then
  if [[ ":$PATH:" != *":$TERMPLEX_BIN_DIR:"* ]]; then
    export PATH="$PATH:$TERMPLEX_BIN_DIR"
  fi
fi

# Sudo
if [[ "$TERMPLEX_SHELL_FEATURES" == *"sudo"* && -n "$TERMINFO" ]]; then
  # Wrap `sudo` command to ensure Termplex terminfo is preserved.
  #
...
# SSH Integration
if [[ "$TERMPLEX_SHELL_FEATURES" == *ssh-* ]]; then
  function ssh() {
    ...
    if [[ "$TERMPLEX_SHELL_FEATURES" == *ssh-terminfo* ]]; then
      ...
      if [[ -n "$ssh_hostname" ]]; then
        ...
        if "$TERMPLEX_BIN_DIR/termplex" +ssh-cache --host="$ssh_target" >/dev/null 2>&1; then
          ssh_term="xterm-termplex"
        elif builtin command -v infocmp >/dev/null 2>&1; then
          builtin local ssh_terminfo ssh_cpath_dir ssh_cpath

          ssh_terminfo=$(infocmp -0 -x xterm-termplex 2>/dev/null)

          if [[ -n "$ssh_terminfo" ]]; then
            builtin echo "Setting up xterm-termplex terminfo on $ssh_hostname..." >&2

            ssh_cpath_dir=$(mktemp -d "/tmp/termplex-ssh-$ssh_user.XXXXXX" 2>/dev/null) || ssh_cpath_dir="/tmp/termplex-ssh-$ssh_user.$$"
            ...
            ' 2>/dev/null; then
              ...
              "$TERMPLEX_BIN_DIR/termplex" +ssh-cache --add="$ssh_target" >/dev/null 2>&1 || true
            else
          ...
          builtin echo "Warning: termplex command not available for cache management." >&2
        fi
      fi
    fi
...
_termplex_executing=""
_termplex_last_reported_cwd=""

function __termplex_precmd() {
  local ret="$?"
  if test "$_termplex_executing" != "0"; then
    _TERMPLEX_SAVE_PS1="$PS1"
    _TERMPLEX_SAVE_PS2="$PS2"
    ...
    # Cursor
    if [[ "$TERMPLEX_SHELL_FEATURES" == *"cursor"* ]]; then
      builtin local cursor=5  # blinking bar
      [[ "$TERMPLEX_SHELL_FEATURES" == *"cursor:steady"* ]] && cursor=6  # steady bar
    ...
    if [[ "$TERMPLEX_SHELL_FEATURES" == *"title"* ]]; then
      PS1=$PS1'\[\e]2;\w\a\]'
    fi
  fi
  ...
  if [[ "$_termplex_last_reported_cwd" != "$PWD" ]]; then
    _termplex_last_reported_cwd="$PWD"
    builtin printf "\e]7;kitty-shell-cwd://%s%s\a" "$HOSTNAME" "$PWD"
  fi

  _termplex_executing=0
}

function __termplex_preexec() {
  builtin local cmd="$1"

  PS1="$_TERMPLEX_SAVE_PS1"
  PS2="$_TERMPLEX_SAVE_PS2"

  # Title (current command)
  if [[ -n $cmd && "$TERMPLEX_SHELL_FEATURES" == *"title"* ]]; then
...
if (( BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 4) )); then
  ...
  if [[ -z "${__termplex_ps0+x}" ]]; then
    if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
      # shellcheck disable=SC2016
      builtin readonly __termplex_ps0='${ __termplex_preexec_hook; }'
    else
      # shellcheck disable=SC2016
      builtin readonly __termplex_ps0='$(__termplex_preexec_hook >/dev/tty)'
    fi
  fi

  __termplex_hook() {
    builtin local ret=$?
    __termplex_precmd "$ret"
    if [[ "$PS0" != *"$__termplex_ps0"* ]]; then
      PS0=$PS0"${__termplex_ps0}"
    fi
  }

  # Append our hook to PROMPT_COMMAND, preserving its existing type.
  # shellcheck disable=SC2128,SC2178,SC2179
  if [[ ";${PROMPT_COMMAND[*]:-};" != *";__termplex_hook;"* ]]; then
    if [[ -z "${PROMPT_COMMAND[*]}" ]]; then
      if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 1) )); then
        PROMPT_COMMAND=(__termplex_hook)
      else
        PROMPT_COMMAND="__termplex_hook"
      fi
  ...
  preexec_functions+=(__termplex_preexec)
  precmd_functions+=(__termplex_precmd)
fi
