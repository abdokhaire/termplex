...
function termplex_restore_xdg_data_dir -d "restore the original XDG_DATA_DIR value"
    # If we don't have our own data dir then we don't need to do anything.
    if not set -q TERMPLEX_SHELL_INTEGRATION_XDG_DIR
        return
    end
    ...
    if set --function index (contains --index "$TERMPLEX_SHELL_INTEGRATION_XDG_DIR" $xdg_data_dirs)
    ...
    set --erase TERMPLEX_SHELL_INTEGRATION_XDG_DIR
end

function termplex_exit -d "exit the shell integration setup"
    functions -e termplex_restore_xdg_data_dir
    functions -e termplex_exit
    exit 0
end

# We always try to restore the XDG data dir
termplex_restore_xdg_data_dir

# If we aren't interactive or we've already run, don't run.
status --is-interactive || termplex_exit
...
function __termplex_setup --on-event fish_prompt -d "Setup termplex integration"
    functions -e __termplex_setup

    set --local features (string split , $TERMPLEX_SHELL_FEATURES)
    ...
    set -g __termplex_prompt_start_mark "\e]133;A\a"
    if test "$fish_major" -gt 4; or test "$fish_major" -eq 4 -a "$fish_minor" -ge 1
        set -g __termplex_prompt_start_mark "\e]133;A;click_events=1\a"
    end
    ...
    if string match -q 'cursor*' -- $features
        ...
        function __termplex_set_cursor_beam --on-event fish_prompt -V cursor -d "Set cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[$cursor q"
            end
        end
        function __termplex_reset_cursor --on-event fish_preexec -d "Reset cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[0 q"
            end
        end
    ...
    if contains path $features; and test -n "$TERMPLEX_BIN_DIR"
        fish_add_path --global --path --append "$TERMPLEX_BIN_DIR"
    end
    ...
    # SSH Integration
    set -l features (string split ',' -- "$TERMPLEX_SHELL_FEATURES")
    if contains ssh-env $features; or contains ssh-terminfo $features
        function ssh --wraps=ssh --description "SSH wrapper with Termplex integration"
            set -l features (string split ',' -- "$TERMPLEX_SHELL_FEATURES")
            ...
            if contains ssh-terminfo $features
                ...
                if test -n "$ssh_hostname"
                    ...
                    if test -x "$TERMPLEX_BIN_DIR/termplex"; and "$TERMPLEX_BIN_DIR/termplex" +ssh-cache --host="$ssh_target" >/dev/null 2>&1
                        set ssh_term xterm-termplex
                    else if command -q infocmp
                        set -l ssh_terminfo
                        set -l ssh_cpath_dir
                        set -l ssh_cpath

                        set ssh_terminfo "$(infocmp -0 -x xterm-termplex 2>/dev/null)"

                        if test -n "$ssh_terminfo"
                            echo "Setting up xterm-termplex terminfo on $ssh_hostname..." >&2

                            set ssh_cpath_dir (mktemp -d "/tmp/termplex-ssh-$ssh_user.XXXXXX" 2>/dev/null; or echo "/tmp/termplex-ssh-$ssh_user."(random))
                            ...
                            ' 2>/dev/null
                                ...
                                if test -x "$TERMPLEX_BIN_DIR/termplex"
                                    "$TERMPLEX_BIN_DIR/termplex" +ssh-cache --add="$ssh_target" >/dev/null 2>&1; or true
                                end
                            else
                        ...
                        echo "Warning: termplex command not available for cache management." >&2
                    end
                end
            end
    ...
    function __termplex_mark_prompt_start --on-event fish_prompt --on-event fish_posterror
        # If we never got the output end event, then we need to send it now.
        if test "$__termplex_prompt_state" != prompt-start
            echo -en "\e]133;D\a"
        end

        set --global __termplex_prompt_state prompt-start
        echo -en $__termplex_prompt_start_mark
    end

    function __termplex_mark_output_start --on-event fish_preexec
    ...
    # Enable fish to handle reflow because Termplex clears the prompt on resize.
    set --global fish_handle_reflow 1

    # Initial calls for first prompt
    if string match -q 'cursor*' -- $features
        __termplex_set_cursor_beam
    end
    __termplex_mark_prompt_start
    __update_cwd_osc
end
