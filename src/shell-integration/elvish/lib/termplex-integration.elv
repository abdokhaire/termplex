{
  ...
  if (and (has-env TERMPLEX_SHELL_INTEGRATION_XDG_DIR) (has-env XDG_DATA_DIRS)) {
    set-env XDG_DATA_DIRS (str:replace $E:TERMPLEX_SHELL_INTEGRATION_XDG_DIR":" "" $E:XDG_DATA_DIRS)
    unset-env TERMPLEX_SHELL_INTEGRATION_XDG_DIR
  }

  # List of enabled shell integration features
  var features = [(str:split ',' $E:TERMPLEX_SHELL_FEATURES)]

  # State tracking for semantic prompt sequences
  # Values: 'prompt-start', 'pre-exec', 'post-exec'
  fn set-prompt-state {|new| set-env __termplex_prompt_state $new }

  fn mark-prompt-start {
    if (not-eq $E:__termplex_prompt_state 'prompt-start') {
  ...
  fn ssh-integration {|@args|
    ...
    if (has-value $features ssh-terminfo) {
      ...
      if (not-eq $ssh-hostname "") {
        var termplex = $E:TERMPLEX_BIN_DIR/"termplex"
        var ssh-target = $ssh-user"@"$ssh-hostname

        # Check if terminfo is already cached
        if (bool ?($termplex +ssh-cache --host=$ssh-target)) {
          set ssh-term = "xterm-termplex"
        } elif (has-external infocmp) {
          var ssh-terminfo = ((external infocmp) -0 -x xterm-termplex 2>/dev/null | slurp)

          if (not-eq $ssh-terminfo "") {
            echo "Setting up xterm-termplex terminfo on "$ssh-hostname"..." >&2

            use os
            var ssh-cpath-dir = (os:temp-dir "termplex-ssh-"$ssh-user".*")
            ...
            if (bool ?(echo $ssh-terminfo | (external ssh) $@ssh-opts -o ControlMaster=yes -o ControlPath=$ssh-cpath -o ControlPersist=60s $@args '
              ...
              $termplex +ssh-cache --add=$ssh-target >/dev/null
            } else {
  ...
  if (str:contains $E:TERMPLEX_SHELL_FEATURES "cursor") {
  ...
  if (and (has-value $features path) (has-env TERMPLEX_BIN_DIR)) {
    if (not (has-value $paths $E:TERMPLEX_BIN_DIR)) {
        set paths = [$@paths $E:TERMPLEX_BIN_DIR]
    }
  }
  if (and (has-value $features sudo) (not-eq "" $E:TERMINFO) (has-external sudo)) {
    edit:add-var sudo~ $sudo-with-terminfo~
  }
  if (and (str:contains $E:TERMPLEX_SHELL_FEATURES ssh-) (has-external ssh)) {
