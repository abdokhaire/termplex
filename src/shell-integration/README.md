...
our integration script (`bash/termplex.bash`). This prevents Bash from loading
its normal startup files, which becomes our script's responsibility (along with
disabling POSIX mode).

Bash shell integration can also be sourced manually from `bash/termplex.bash`.
This also works for older versions of Bash.

```bash
# Termplex shell integration for Bash. This must be at the top of your bashrc!
if [ -n "${TERMPLEX_RESOURCES_DIR}" ]; then
    builtin source "${TERMPLEX_RESOURCES_DIR}/shell-integration/bash/termplex.bash"
fi
```
...
For [Elvish](https://elv.sh), `$TERMPLEX_RESOURCES_DIR/src/shell-integration`
contains an `./elvish/lib/termplex-integration.elv` file.
...
Termplex launches Elvish, passing the environment with `XDG_DATA_DIRS` prepended
with `$TERMPLEX_RESOURCES_DIR/src/shell-integration`. It contains
`./elvish/lib/termplex-integration.elv`. The user can then import it
by `use termplex-integration` every time after shell startup or
autostart integration in `$XDG_CONFIG_HOME/elvish/rc.elv`,
which will run the integration routines.

If you decide to autostart `termplex-integration` with `rc.elv`, you should
detect whether the terminal is Termplex or not. To do this, add this to the end
of your `rc.elv` file:

```elvish
if (eq $E:TERM "xterm-termplex") {
  try { use termplex-integration } catch { }
}
```

The [Elvish](https://elv.sh) shell integration is supported by
the community and is not officially supported by Termplex. We distribute
...
For [Fish](https://fishshell.com/), Termplex prepends to the
...
For [Nushell](https://www.nushell.sh/), Termplex prepends to the
`XDG_DATA_DIRS` directory, making the `termplex` module available through
Nushell's vendor autoload mechanism. Termplex then automatically imports
the module using the `-e "use termplex *"` flag when starting Nushell.

Nushell provides many shell features itself, such as `title` and `cursor`,
so our integration focuses on Termplex-specific features like `sudo`,
`ssh-env`, and `ssh-terminfo`.

The shell integration is automatically enabled when running Nushell in Termplex,
but you can also load it manually is shell integration is disabled:

```nushell
source $TERMPLEX_RESOURCES_DIR/shell-integration/nushell/vendor/autoload/termplex.nu
use termplex *
```
...
override Termplex's `ZDOTDIR` value, preventing the shell integration from being
...
```zsh
if [[ -n $TERMPLEX_RESOURCES_DIR ]]; then
  source "$TERMPLEX_RESOURCES_DIR"/shell-integration/zsh/termplex-integration
fi
```
