...
pub const Flags = packed struct {
    ...
    /// and not just while Termplex is focused. This may not work on all platforms.
    ...
    ///
    /// Sync with termplex.h for enums.
...
pub const Action = union(enum) {
    ...
    ///
    /// Termplex will not process this combination nor forward it to the child
    ...
    ///
    /// This cannot unbind bindings that were not bound by Termplex or the user
    ...
    /// found by running `termplex +version`.
    toggle_tab_overview,
    ...
    /// You must manually untoggle it or quit Termplex entirely to disable it.
    ///
    ...
    /// version can be found by running `termplex +version`.
    ...
    ///     plugin in the plugin list. Termplex would then need to be restarted
    ...
    /// Termplex becomes focused. When hiding all windows, focus is yielded
    ...
    /// windows.
    ///
    /// Not every action in Termplex can be undone or redone. The list
    ...
    /// Crash Termplex in the desired thread for the focused surface.
    ///
    ...
    pub const WriteScreen = struct {
        ...
        pub fn parse(param: []const u8) !WriteScreen {
            ...
            // also very important for backwards compatibility before Termplex
...
///
/// This must be kept in sync with include/termplex.h termplex_input_trigger_s
pub const Trigger = struct {
    ...
    pub const Key = union(C.Tag) {
        ...
        /// registered keycode with Termplex.
        unicode: u21,
    ...
    pub fn parse(input: []const u8) !Trigger {
        ...
        loop: while (rem.len > 0) {
            ...
            // keys with Termplex 1.1.x. We do this last so its least likely
    ...
    /// The values that are backwards compatible with Termplex 1.1.x.
    /// Termplex 1.2+ doesn't support these anymore since we moved to
    /// W3C key codes.
...
// For Termplex 1.2+ we changed our key names to match the W3C and removed
